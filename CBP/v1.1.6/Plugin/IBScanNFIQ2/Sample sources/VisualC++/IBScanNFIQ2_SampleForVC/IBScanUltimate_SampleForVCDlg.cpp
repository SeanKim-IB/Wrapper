...
void CIBScanUltimate_SampleForVCDlg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHECK_NFIQ2, m_chkUseNFIQ2);
}
...
//	IBSU_GetSDKVersion(&m_verInfo);
    char nfiq2_ver[256];
    IBSU_NFIQ2_GetVersion(nfiq2_ver);
	CString titleName;
	titleName.Format("NFIQ2 Sample for VC++");
	SetWindowText(titleName);

	CString strDrawString;
    strDrawString.Format("VC++ sample with DLL ver. %s", nfiq2_ver);
...
{   
	...
	{
		nRc = IBSU_OpenDeviceEx( devIndex, ".\\", FALSE, &devHandle );
	}
	else
	{
...
*/
	    ...
	    if( pDlg->m_chkUseNFIQ2 )
	    {
			...
			strValue.Format("%d-%d-%d-%d", nfiq_score2[0], nfiq_score2[1], nfiq_score2[2], nfiq_score2[3]);
			pDlg->GetDlgItem( IDC_EDIT_NFIQ2 )->SetWindowText(strValue);
        ...
        {
            // > IBSU_STATUS_OK
	        pDlg->m_strImageMessage.Format(_T("%s acquisition Warning (Warning code = %d)\n"), imgTypeName, imageStatus);
			
			BOOL isInvalidDetected = FALSE;
			if( (imageStatus & IBSU_WRN_QUALITY_INVALID_AREA_HORIZONTALLY) == IBSU_WRN_QUALITY_INVALID_AREA_HORIZONTALLY)
			{
				pDlg->m_strImageMessage.Append(" Horizontal");
				isInvalidDetected = TRUE;
			}

			if( (imageStatus & IBSU_WRN_QUALITY_INVALID_AREA_VERTICALLY) == IBSU_WRN_QUALITY_INVALID_AREA_VERTICALLY)
			{
				pDlg->m_strImageMessage.Append(" Vertical");
				isInvalidDetected = TRUE;
			}

			if(isInvalidDetected)
				pDlg->m_strImageMessage.Append(" invalid area.");
...
LRESULT CIBScanUltimate_SampleForVCDlg::OnMsg_AskRecapture(WPARAM wParam, LPARAM lParam)
{
	...
	if( (imageStatus & IBSU_WRN_QUALITY_INVALID_AREA_HORIZONTALLY) == IBSU_WRN_QUALITY_INVALID_AREA_HORIZONTALLY)
		askMsg.Append("\nHorizontal invalid area");
	if( (imageStatus & IBSU_WRN_QUALITY_INVALID_AREA_VERTICALLY) == IBSU_WRN_QUALITY_INVALID_AREA_VERTICALLY)
		askMsg.Append("\nVertical invalid area");
...
void CIBScanUltimate_SampleForVCDlg::OnBnClickedBtnCaptureStart()
{
	...
	if( m_nDevHandle == -1 )
	{
		...
		if( m_chkAsyncOpenDevice )
		{
			...
			if( m_chkOpenDeviceEx )
				nRc = IBSU_OpenDeviceEx( devIndex, ".\\", TRUE, &tmpDevHandle/* Always vaule is "-1" when async mode"*/ );
			else
...
HBRUSH CIBScanUltimate_SampleForVCDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	//IBSU_ModifyOverlayLine(m_nDevHandle,
