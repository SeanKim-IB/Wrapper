	...
	const NFIQ::FingerprintImageData & fingerprintImage, unsigned char templateData[], size_t & templateSize, bool & templateCouldBeExtracted)
{
	...
	try
	{
		...
		// validity check
		if (templateSize < 28)
			throw NFIQ::NFIQException(NFIQ::e_Error_FeatureCalculationError, "Created ISO template is too small.");

		struct finger_minutiae_record *fmr;
		if (new_fmr(FMR_STD_ISO, &fmr) < 0) 
			throw NFIQ::NFIQException(NFIQ::e_Error_FeatureCalculationError, "Could not allocate FMR.");
		...
		if (scan_fmr(&bdb, fmr) != READ_OK)
		{
			if (fmr != NULL)
				free_fmr(fmr);
			throw NFIQ::NFIQException(NFIQ::e_Error_FeatureCalculationError, "Could not init BDB.");
		}
		...
		if (rcount == 0)
		{
			if (fmr != NULL)
				free_fmr(fmr);
			throw NFIQ::NFIQException(NFIQ::e_Error_FeatureCalculationError, "No minutiae records found.");
		}
		...
		if (get_fvmrs(fmr, fvmrs) != rcount)
		{
			...
			if (fmr != NULL)
				free_fmr(fmr);
			throw NFIQ::NFIQException(NFIQ::e_Error_FeatureCalculationError, "Cannot get minutiae records.");
		}
		...
		if (get_fmds(fvmrs[0], fmds) != minCnt)
		{
			...
			if (fmr != NULL)
				free_fmr(fmr);
			throw NFIQ::NFIQException(NFIQ::e_Error_FeatureCalculationError, "Cannot get minutiae data.");
		}

		// compute minutiae quality based on Mu feature computated at minutiae positions
		std::vector<MinutiaData> vecMuMinQualityData = computeMuMinQuality(fmds, minCnt, 32, fingerprintImage);

		std::vector<unsigned int> vecRanges(4); // index 0 = -1 .. -0.5, ....
		...
		for (unsigned int i = 0; i < vecMuMinQualityData.size(); i++)
		{
			if (vecMuMinQualityData.at(i).quality <= -0.5)
				vecRanges.at(0)++;
			else if (vecMuMinQualityData.at(i).quality > -0.5 && vecMuMinQualityData.at(i).quality <= 0.0)
				vecRanges.at(1)++;
			else if (vecMuMinQualityData.at(i).quality > 0.0 && vecMuMinQualityData.at(i).quality <= 0.5)
				vecRanges.at(2)++;
		...
		res_mu.featureData.featureDataDouble = (double)vecRanges.at(2) / (double)minCnt;
		featureDataList.push_back(res_mu);

		// compute minutiae quality based on OCL feature computed at minutiae positions
		std::vector<MinutiaData> vecOCLMinQualityData = computeOCLMinQuality(fmds, minCnt, BS_OCL, fingerprintImage);

		std::vector<unsigned int> vecRangesOCL(5); // index 0 = 0-20, 1 = 20-40, ..., 5 = 80-100
		...
		for (unsigned int i = 0; i < vecOCLMinQualityData.size(); i++)
		{
			if (vecOCLMinQualityData.at(i).quality <= 20)
				vecRangesOCL.at(0)++;
			else if (vecOCLMinQualityData.at(i).quality > 20 && vecOCLMinQualityData.at(i).quality <= 40)
				vecRangesOCL.at(1)++;
			else if (vecOCLMinQualityData.at(i).quality > 40 && vecOCLMinQualityData.at(i).quality <= 60)
				vecRangesOCL.at(2)++;
			else if (vecOCLMinQualityData.at(i).quality > 60 && vecOCLMinQualityData.at(i).quality <= 80)
				vecRangesOCL.at(3)++;
		...
		res_ocl.returnCode = 0;
		// return relative value in relation to minutiae count
		res_ocl.featureData.featureDataDouble = (double)vecRangesOCL.at(4) / (double)minCnt;
		featureDataList.push_back(res_ocl);
	...
	catch (cv::Exception & e)
	{
		...
		ssErr << "Cannot compute FJFX based minutiae quality features: " << e.what();
	...
	return featureDataList;
#else
	throw NFIQ::NFIQException(NFIQ::e_Error_FunctionNotImplemented, "libbiomdi support is not enabled. FJFX minutiae quality feature computation is not possible.");
#endif
}
	...
	int bs, const NFIQ::FingerprintImageData & fingerprintImage)
{
	...
	for (unsigned int i = 0; i < minCount; i++)
	{
		...
		// if in edge reason -> don't center around minutiae but take full block that is closest
		if ((leftX + bs) > (int)fingerprintImage.m_ImageWidth)
		...
		// ignore return value as if false is returned OCL value is 0 anyway
		double ocl = 0.0;
