...
ceusbkwrapper_API HANDLE UkwOpenDriver()
{
    ...
    FUNCTION_MSG((TEXT("USBKWrapper!UkOpenDriver DeviceName=\"%s\" LegacyName=\"%s\"\r\n"), di.szDeviceName, di.szLegacyName));
	...
	)
{
	BOOL bRet = 0;
	ENTRYPOINT_MSG((
		TEXT("USBKWrapper!UkwIssueBulkTransfer(lpDevice=0x%08x, dwFlags=%08x, EndPoint=%02x, lpDataBuffer=%08x, BufSize=%08x, lpOverlapped=%08x, ...)\r\n"),
		lpDevice, dwFlags, Endpoint, lpDataBuffer, dwDataBufferSize, lpOverlapped));
	...
	bRet = DeviceIoControl(
		...
		NULL, NULL, NULL, NULL);
	FUNCTION_MSG((
		TEXT("USBKWrapper!UkwIssueBulkTransfer returning %s\r\n"),
