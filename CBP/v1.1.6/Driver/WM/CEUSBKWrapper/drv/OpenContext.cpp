...
{
	if (!device.Valid()){
		ERROR_MSG((TEXT("USBKWrapperDrv!OpenContext::Validate() invalid DevicePtr\r\n")));
		return FALSE;
	}
...
BOOL OpenContext::StartBulkTransfer(LPUKWD_BULK_TRANSFER_INFO lpTransferInfo)
{
	...
	if (dev->Closed()) {
		ERROR_MSG((TEXT("USBKWrapperDrv!OpenContext::StartBulkTransfer() - ")
			TEXT("on device 0x%08x when it's closed\r\n"),
			dev));
		SetLastError(ERROR_INVALID_HANDLE);
		return FALSE;
	}
...
BOOL OpenContext::ClearHaltDevice(LPUKWD_ENDPOINT_INFO lpEndpointInfo)
{
	...
	DevicePtr dev (mDevice->GetDeviceList(), lpEndpointInfo->lpDevice);
	if (!Validate(dev)) {
		ERROR_MSG((TEXT("USBKWrapperDrv!OpenContext::ClearHaltDevice failed validatioin\r\n")));
		SetLastError(ERROR_INVALID_HANDLE);
		return FALSE;
	}
	...
	// Interface can't be used if device has closed
	if (dev->Closed()) {
		ERROR_MSG((TEXT("USBKWrapperDrv!OpenContext::ClearHaltDevice already closed\r\n")));
		SetLastError(ERROR_INVALID_HANDLE);
		return FALSE;
	}
...
BOOL OpenContext::ResetDevice(UKWD_USB_DEVICE DeviceIdentifier)
{
	...
	DevicePtr dev (mDevice->GetDeviceList(), DeviceIdentifier);
	if (!Validate(dev)) {
		ERROR_MSG((TEXT("USBKWrapperDrv!OpenContext::ResetDevice Validate dev failed\r\n")));		
		SetLastError(ERROR_INVALID_HANDLE);
		return FALSE;
	}
	return dev->Reset();
...
BOOL OpenContext::ReenumerateDevice(UKWD_USB_DEVICE DeviceIdentifier)
{
	...
	DevicePtr dev (mDevice->GetDeviceList(), DeviceIdentifier);
	if (!Validate(dev)) {
		ERROR_MSG((TEXT("USBKWrapperDrv!OpenContext::ReenumerateDevice Validate dev failed\r\n")));
		SetLastError(ERROR_INVALID_HANDLE);
		return FALSE;
	}
...
BOOL OpenContext::IsKernelDriverActiveForInterface(LPUKWD_INTERFACE_INFO lpInterfaceInfo, PBOOL active)
{
	...
	DevicePtr dev (mDevice->GetDeviceList(), lpInterfaceInfo->lpDevice);
	if (!Validate(dev)) {
		ERROR_MSG((TEXT("USBKWrapperDrv!OpenContext::IsKernelDriverActiveForInterface Validate dev failed\r\n")));
		SetLastError(ERROR_INVALID_HANDLE);
		return FALSE;
	}
...
BOOL OpenContext::AttachKernelDriverForInterface(LPUKWD_INTERFACE_INFO lpInterfaceInfo)
{
	...
	DevicePtr dev (mDevice->GetDeviceList(), lpInterfaceInfo->lpDevice);
	if (!Validate(dev)) {
		ERROR_MSG((TEXT("USBKWrapperDrv!OpenContext::AttachKernelDriverForInterface Validate dev failed\r\n")));
		SetLastError(ERROR_INVALID_HANDLE);
		return FALSE;
	}
...
BOOL OpenContext::DetachKernelDriverForInterface(LPUKWD_INTERFACE_INFO lpInterfaceInfo)
{
	...
	DevicePtr dev (mDevice->GetDeviceList(), lpInterfaceInfo->lpDevice);
	if (!Validate(dev)) {
		ERROR_MSG((TEXT("USBKWrapperDrv!OpenContext::DetachKernelDriverForInterface Validate dev failed\r\n")));
		SetLastError(ERROR_INVALID_HANDLE);
		return FALSE;
	}
