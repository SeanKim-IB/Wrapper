...
CIBScanUltimate_SampleForVCDlg::CIBScanUltimate_SampleForVCDlg(CWnd* pParent /*=NULL*/)
	...
	, m_chkUseStartVpp(FALSE)
	, m_strSharpVal(_T("1.0"))
	, m_nAgingTest(FALSE)
	, m_TOTAL_CNT(0)
	, m_SUCCESS_CNT(0)
{
...
void CIBScanUltimate_SampleForVCDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_ENABLE_STARTING_VOLTAGE, m_chkUseStartVpp);
	DDX_Control(pDX, IDC_SLIDER_STARTING_VOLTAGE, m_sliderVoltage);
	DDX_Text(pDX, IDC_EDIT_STARTING_VOLTAGE2, m_strSharpVal);
	DDX_Control(pDX, IDC_SLIDER_WET_DETECT_LEVEL, m_sliderWetLevel);
}
...
BEGIN_MESSAGE_MAP(CIBScanUltimate_SampleForVCDlg, CDialog)
	...
	ON_WM_CLOSE()
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_STARTING_VOLTAGE, &CIBScanUltimate_SampleForVCDlg::OnNMCustomdrawSliderStartingVoltage)
	ON_NOTIFY(NM_RELEASEDCAPTURE, IDC_SLIDER_STARTING_VOLTAGE, &CIBScanUltimate_SampleForVCDlg::OnNMReleasedcaptureSliderStartingVoltage)
	ON_BN_CLICKED(IDC_CHECK_ENABLE_STARTING_VOLTAGE, &CIBScanUltimate_SampleForVCDlg::OnBnClickedCheckEnableStartingVoltage)
	ON_EN_CHANGE(IDC_EDIT_STARTING_VOLTAGE2, &CIBScanUltimate_SampleForVCDlg::OnEnChangeEditStartingVoltage2)
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_WET_DETECT_LEVEL, &CIBScanUltimate_SampleForVCDlg::OnNMCustomdrawSliderWetDetectLevel)
	ON_NOTIFY(NM_RELEASEDCAPTURE, IDC_SLIDER_WET_DETECT_LEVEL, &CIBScanUltimate_SampleForVCDlg::OnNMReleasedcaptureSliderWetDetectLevel)
	ON_BN_CLICKED(IDC_RADIO_TOF_ON, &CIBScanUltimate_SampleForVCDlg::OnBnClickedRadioTofOn)
	ON_BN_CLICKED(IDC_RADIO_TOF_OFF, &CIBScanUltimate_SampleForVCDlg::OnBnClickedRadioTofOff)
	ON_BN_CLICKED(IDC_RADIO_TOF_ON_ROLL, &CIBScanUltimate_SampleForVCDlg::OnBnClickedRadioTofOnRoll)
	ON_BN_CLICKED(IDC_RADIO_TOF_OFF_ROLL, &CIBScanUltimate_SampleForVCDlg::OnBnClickedRadioTofOffRoll)
	ON_BN_CLICKED(IDC_BUTTON_AGING_TEST, &CIBScanUltimate_SampleForVCDlg::OnBnClickedButtonAgingTest)
	ON_BN_CLICKED(IDC_BUTTON_GET_FINAL_IMAGE_BY_NATIVE, &CIBScanUltimate_SampleForVCDlg::OnBnClickedButtonGetFinalImageByNative)
END_MESSAGE_MAP()
...
BOOL CIBScanUltimate_SampleForVCDlg::OnInitDialog()
{
	...
	m_sliderVoltage.SetRange(0xA0, 0xFF);
	m_sliderVoltage.SetPos((0xFF-0xC9) + 0xA0);
	m_sliderVoltage.SetTicFreq(5);
	GetDlgItem(IDC_EDIT_STARTING_VOLTAGE)->SetWindowText("C9");

	m_sliderWetLevel.SetRange(1, 5);
	m_sliderWetLevel.SetPos(3);
	GetDlgItem(IDC_EDIT_WET_DETECT_LEVEL)->SetWindowText("3");
	...
	strDrawString.Format("VC++ sample with DLL ver. %s beta", m_verInfo.Product);
	...
	((CButton*)GetDlgItem(IDC_RADIO_TOF_ON))->SetCheck(1);
	((CButton*)GetDlgItem(IDC_RADIO_TOF_ON_ROLL))->SetCheck(1);
					   ...
					   )
{
	...
	int              pRollingWidth;
	IBSU_BGetRollingInfoEx(deviceHandle, &pRollingState, &pRollingLineX, &pRollingDirection, &pRollingWidth);

	message.Format(_T("%s (%.1f fps), %d,%d,%d,%d"), pDlg->m_strImageMessage, 1000/(image.FrameTime*1000),
						pRollingState, pRollingLineX, pRollingDirection, pRollingWidth);
...
//    IBSU_SetBeeper(deviceHandle, ENUM_IBSU_BEEP_PATTERN_REPEAT, 2/*Sol*/, 3/*75ms = 3*25ms*/, 4/*100ms = 4*25ms*/, 0 /*stop*/);
	...
	int width_mm=0;
	IBSU_CheckWetFinger(deviceHandle, pSegmentImageArray[0]);
	IBSU_GetImageWidth(deviceHandle, pSegmentImageArray[0], &width_mm);
...
/*	if(m_chkUseStartVpp)
	{
		RESERVED_SetProperty(m_nDevHandle, "ibkorea1120!", ENUM_IBSU_PROPERTY_RESERVED_ENABLE_STARTING_VOLTAGE, "TRUE");
	}
	else
	{
		RESERVED_SetProperty(m_nDevHandle, "ibkorea1120!", ENUM_IBSU_PROPERTY_RESERVED_ENABLE_STARTING_VOLTAGE, "FALSE");
	}

	int CurDAC = (0xFF-m_sliderVoltage.GetPos()) + 0xA0;

	CString str;
	str.Format("%X", CurDAC);
	GetDlgItem(IDC_EDIT_STARTING_VOLTAGE)->SetWindowText(str);

	str.Format("%d", 0xFF-CurDAC+1);
	RESERVED_SetProperty(m_nDevHandle, "ibkorea1120!", ENUM_IBSU_PROPERTY_RESERVED_STARTING_VOLTAGE_VALUE, str);

	RESERVED_SetProperty(m_nDevHandle, "ibkorea1120!", ENUM_IBSU_PROPERTY_RESERVED_SHARPENING_VALUE, m_strSharpVal);

	IBSU_SetProperty(m_nDevHandle, ENUM_IBSU_PROPERTY_ENABLE_WET_FINGER_DETECT, "TRUE");
	str.Format("%d", m_sliderWetLevel.GetPos());
	IBSU_SetProperty(m_nDevHandle, ENUM_IBSU_PROPERTY_WET_FINGER_DETECT_LEVEL, str);
*/
	//IBSU_SetProperty(m_nDevHandle, ENUM_IBSU_PROPERTY_ROLL_IMAGE_OVERRIDE, "TRUE");
	//IBSU_SetProperty(m_nDevHandle, ENUM_IBSU_PROPERTY_NO_PREVIEW_IMAGE, "TRUE");

	//IBSU_SetProperty(m_nDevHandle, ENUM_IBSU_PROPERTY_START_ROLL_WITHOUT_LOCK, "TRUE");
	IBSU_SetProperty(m_nDevHandle, ENUM_IBSU_PROPERTY_WET_FINGER_DETECT_LEVEL_THRESHOLD, "50 100 150 200 250");
	RESERVED_SetProperty(m_nDevHandle, "ibkorea1120!", ENUM_IBSU_PROPERTY_RESERVED_CAPTURE_BRIGHTNESS_THRESHOLD_FOR_FLAT, "95 100");
...
//        }
		...
		if(m_nAgingTest)
		{
			/*FILE *fp = fopen("TOF_log.log", "at");
			fprintf(fp, "\n[%d/%d] IBSU_BeginCaptureImage : SUCCESS\n", ++m_SUCCESS_CNT, ++m_TOTAL_CNT);
			fclose(fp);*/

			CString str;
			str.Format("[%d/%d] IBSU_BeginCaptureImage : SUCCESS\n", ++m_SUCCESS_CNT, ++m_TOTAL_CNT);
			SetWindowText(str);
		}
    }
	else
	{
		...
		{
			/*FILE *fp = fopen("TOF_log.log", "at");
			fprintf(fp, "\n[%d/%d] IBSU_BeginCaptureImage : FAILED\n", m_SUCCESS_CNT, ++m_TOTAL_CNT);
			fclose(fp);*/
			CString str;
			str.Format("[%d/%d] IBSU_BeginCaptureImage : FAILED\n", m_SUCCESS_CNT, ++m_TOTAL_CNT);
			SetWindowText(str);
		}
	}
...
LRESULT CIBScanUltimate_SampleForVCDlg::OnMsg_UpdateDisplayResources(WPARAM wParam, LPARAM lParam)
{
	...
	GetDlgItem( IDC_RADIO_TOF_ON )->EnableWindow( (m_nDevHandle==-1)?0:1 && idle);
	GetDlgItem( IDC_RADIO_TOF_OFF )->EnableWindow( (m_nDevHandle==-1)?0:1 && idle);
	GetDlgItem( IDC_RADIO_TOF_ON_ROLL )->EnableWindow( (m_nDevHandle==-1)?0:1 && idle);
	GetDlgItem( IDC_RADIO_TOF_OFF_ROLL )->EnableWindow( (m_nDevHandle==-1)?0:1 && idle);

	GetDlgItem( IDC_BUTTON_AGING_TEST )->SetWindowTextA((m_nAgingTest==TRUE)?"Stop":"Aging Test");
...
void CIBScanUltimate_SampleForVCDlg::OnBnClickedCheckAutoContrast()
{
	...
	if( m_chkAutoContrast )
	{
		...
		GetDlgItem(IDC_STATIC_STARTING_VOLTAGE)->EnableWindow(TRUE);
		GetDlgItem(IDC_SLIDER_STARTING_VOLTAGE)->EnableWindow(TRUE);
		GetDlgItem(IDC_EDIT_STARTING_VOLTAGE)->EnableWindow(TRUE);
	}
	else
	{
		...
		GetDlgItem(IDC_STATIC_STARTING_VOLTAGE)->EnableWindow(FALSE);
		GetDlgItem(IDC_SLIDER_STARTING_VOLTAGE)->EnableWindow(FALSE);
		GetDlgItem(IDC_EDIT_STARTING_VOLTAGE)->EnableWindow(FALSE);
	}
}
...
void CIBScanUltimate_SampleForVCDlg::OnTimer(UINT_PTR nIDEvent)
{
	if( nIDEvent == TIMER_STATUS_FINGER_QUALITY )
	{
		...
		if(m_nDevHandle > -1)
		{
			char strTOF[256], strTOFRoll[256];
			IBSU_GetProperty(m_nDevHandle, ENUM_IBSU_PROPERTY_ENABLE_TOF, strTOF);
			IBSU_GetProperty(m_nDevHandle, ENUM_IBSU_PROPERTY_RESERVED_ENABLE_TOF_FOR_ROLL, strTOFRoll);
			
			if(strcmp(strTOF, "TRUE") == 0)
			{
				((CButton*)GetDlgItem(IDC_RADIO_TOF_ON))->SetCheck(1);
				((CButton*)GetDlgItem(IDC_RADIO_TOF_OFF))->SetCheck(0);
			}
			else
			{
				((CButton*)GetDlgItem(IDC_RADIO_TOF_ON))->SetCheck(0);
				((CButton*)GetDlgItem(IDC_RADIO_TOF_OFF))->SetCheck(1);
			}

			if(strcmp(strTOFRoll, "TRUE") == 0)
			{
				((CButton*)GetDlgItem(IDC_RADIO_TOF_ON_ROLL))->SetCheck(1);
				((CButton*)GetDlgItem(IDC_RADIO_TOF_OFF_ROLL))->SetCheck(0);
			}
			else
			{
				((CButton*)GetDlgItem(IDC_RADIO_TOF_ON_ROLL))->SetCheck(0);
				((CButton*)GetDlgItem(IDC_RADIO_TOF_OFF_ROLL))->SetCheck(1);
			}
		}
	}
...
void CIBScanUltimate_SampleForVCDlg::OnNMCustomdrawSliderStartingVoltage(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
...
void CIBScanUltimate_SampleForVCDlg::OnNMReleasedcaptureSliderStartingVoltage(NMHDR *pNMHDR, LRESULT *pResult)
{
	int CurDAC = (0xFF-m_sliderVoltage.GetPos()) + 0xA0;
...
void CIBScanUltimate_SampleForVCDlg::OnBnClickedCheckEnableStartingVoltage()
{
	UpdateData(TRUE);
}

void CIBScanUltimate_SampleForVCDlg::OnEnChangeEditStartingVoltage2()
{
	UpdateData(TRUE);
}

void CIBScanUltimate_SampleForVCDlg::OnNMCustomdrawSliderWetDetectLevel(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	
	int WetLevel = m_sliderWetLevel.GetPos();

	CString str;
	str.Format("%X", WetLevel);
	GetDlgItem(IDC_EDIT_WET_DETECT_LEVEL)->SetWindowText(str);

	*pResult = 0;
}

void CIBScanUltimate_SampleForVCDlg::OnNMReleasedcaptureSliderWetDetectLevel(NMHDR *pNMHDR, LRESULT *pResult)
{
	int WetLevel = m_sliderWetLevel.GetPos();
...
void CIBScanUltimate_SampleForVCDlg::OnBnClickedRadioTofOn()
{
	if(IBSU_SetProperty(m_nDevHandle, ENUM_IBSU_PROPERTY_ENABLE_TOF, "TRUE") != IBSU_STATUS_OK)
	{
...
void CIBScanUltimate_SampleForVCDlg::OnBnClickedRadioTofOff()
{
	if(IBSU_SetProperty(m_nDevHandle, ENUM_IBSU_PROPERTY_ENABLE_TOF, "FALSE") != IBSU_STATUS_OK)
	{
		AfxMessageBox("Not support");
		return;
	}
...
void CIBScanUltimate_SampleForVCDlg::OnBnClickedRadioTofOnRoll()
{
	if(RESERVED_SetProperty(m_nDevHandle, "ibkorea1120!", ENUM_IBSU_PROPERTY_RESERVED_ENABLE_TOF_FOR_ROLL, "TRUE") != IBSU_STATUS_OK)
		AfxMessageBox("Not support");
}

void CIBScanUltimate_SampleForVCDlg::OnBnClickedRadioTofOffRoll()
{
	if(RESERVED_SetProperty(m_nDevHandle, "ibkorea1120!", ENUM_IBSU_PROPERTY_RESERVED_ENABLE_TOF_FOR_ROLL, "FALSE") != IBSU_STATUS_OK)
		AfxMessageBox("Not support");
}

void CIBScanUltimate_SampleForVCDlg::OnBnClickedButtonAgingTest()
{
	m_TOTAL_CNT = 0;
	m_SUCCESS_CNT = 0;

	m_nAgingTest = !m_nAgingTest;

	if(m_nAgingTest)
	{
		AfxBeginThread(_threadAgingTest, this);
	}

	PostMessage( WM_USER_UPDATE_DISPLAY_RESOURCES );
}

UINT CIBScanUltimate_SampleForVCDlg::_threadAgingTest(LPVOID pParam)
{
	CIBScanUltimate_SampleForVCDlg *pDlg = (CIBScanUltimate_SampleForVCDlg*)pParam;

	while(pDlg->m_nAgingTest)
	{
		pDlg->m_cboCaptureSeq.SetCurSel(1);
		pDlg->SendMessage(WM_COMMAND, IDC_BTN_CAPTURE_START);
		Sleep(5000);

		pDlg->SendMessage(WM_COMMAND, IDC_BTN_CAPTURE_STOP);
		Sleep(1000);

		pDlg->m_cboCaptureSeq.SetCurSel(2);
		pDlg->SendMessage(WM_COMMAND, IDC_BTN_CAPTURE_START);
		Sleep(5000);
...
void CIBScanUltimate_SampleForVCDlg::OnBnClickedButtonGetFinalImageByNative()
{
	IBSU_ImageData image;
	if(RESERVED_GetFinalImageByNative(m_nDevHandle, "ibkorea1120!", &image) == IBSU_STATUS_OK)
		IBSU_SaveBitmapImage("d:\\debugging image\\final_image_by_native.bmp", (BYTE*)image.Buffer, image.Width, image.Height, image.Pitch, image.ResolutionX, image.ResolutionY);
	else
		AfxMessageBox("Not Support");
}
