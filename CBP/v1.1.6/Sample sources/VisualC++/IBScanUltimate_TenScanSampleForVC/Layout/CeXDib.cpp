...
{
	if (m_hhMemDc)	::DeleteDC(m_hhMemDc);
...
void CCeXDib::Draw(HDC hDC, DWORD dwX, DWORD dwY)
{
	...
	if (m_hhMemDc == NULL)
	{
		m_hhMemDc = CreateCompatibleDC(hDC);
		if (m_hhMemDc == NULL)	return;
	} // if

	hOldBitmap = (HBITMAP)SelectObject(m_hhMemDc, m_hBitmap);

	BitBlt(hDC, dwX, dwY, m_bi.biWidth, m_bi.biHeight, m_hhMemDc, 0, 0, SRCCOPY);

	SelectObject(m_hhMemDc, hOldBitmap);
} // End of Draw
