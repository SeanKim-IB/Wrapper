...
int CMainCapture::_InitializeForCISRegister(WORD rowStart, WORD colStart, WORD width, WORD height)
{
    switch (m_UsbDeviceInfo.devType)
    {
        case DEVICE_TYPE_WATSON:
...
int CMainCapture::Reserved_WriteEEPROM(WORD addr, const BYTE *buf, int len)
{
    ...
    return _UsbBulkOutIn(EP1OUT, CMD_WRITE_EEPROM, outBuffer, 3 + len, -1, NULL, 0, NULL);
}
...
int	CMainCapture::Reserved_GetCBPPreviewInfo(IBSU_SegmentPosition *segmentPositionArray, IBSU_SegmentPosition *segmentPositionArray_for_geo, int *segmentPositionArrayCount, 
...
int	CMainCapture::Reserved_CleanUpCBP()
{
	m_blockProperty.bGetOneFrameImage = FALSE;
