...
LOCAL_CFLAGS += -D__linux__ -D__embedded__ -D__android__ -D_STLP_USE_NEWALLOC -D__ppi__ -fexceptions \
	-W -Wall

LOCAL_SRC_FILES  :=          \
	CaptureLib/IBEncryption.cpp         \
	CaptureLib/ParallelProc.cpp            \
...
LOCAL_C_INCLUDES             := $(LOCAL_PATH)/. $(LOCAL_PATH)/../include $(LOCAL_PATH)/CaptureLib $(LOCAL_PATH)/ThreadPool $(LOCAL_PATH)/MatcherLib "$(LOCAL_PATH)/../Samples/Android/LibUsb/jni/libusb/compat" "$(LOCAL_PATH)/../../LibDeviceParallel/jni/include" $(LOCAL_PATH)/CxImage_Libs/include
LOCAL_MODULE                 := ibscanultimate
LOCAL_LDLIBS                 := -llog  -Wl,--version-script=$(LOCAL_PATH)/IBScanUltimate.vsc
LOCAL_SHARED_LIBRARIES       += usb
LOCAL_SHARED_LIBRARIES       += DeviceParallel
