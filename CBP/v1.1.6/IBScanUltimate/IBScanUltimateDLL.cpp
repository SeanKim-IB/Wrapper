...
#if !defined(__android__) && !defined(WINCE) && defined(__IBSCAN_ULTIMATE_SDK__)
#include "ximage.h"
#endif
...
#if defined(__android__) || defined(WINCE)
    nRc = IBSU_ERR_NOT_SUPPORTED;
    InsertTraceLog(sTraceMainCategory, nRc, _GetThreadID(),
...
//	CThreadSync Sync;
	...
	InsertTraceLog(sTraceMainCategory, 0, _GetThreadID(),
		"Enter RES_GetSpoofScore (deviceID=%d)", deviceID);
	...
	if (Width != 400 || Height != 500)
	{
		nRc = IBSU_ERR_INVALID_PARAM_VALUE;
		InsertTraceLog(sTraceMainCategory, nRc, _GetThreadID(),
			"Exit RES_GetSpoofScore (deviceID=%d, width=%d, height=%d)", deviceID, Width, Height);
	 ...
	 int                   *pScore)
{
	...
	return theApp.RES_GetSpoofScore(pReservedKey, deviceID, pImage, Width, Height, pScore);
}
#endif
...
int	CIBScanUltimateApp::RES_GetCBPPreviewInfo
    (const int              handle,
     const char            *pReservedkey,
     IBSU_SegmentPosition  *segmentPositionArray,
...
//	CThreadSync Sync;
	...
	InsertTraceLog(sTraceMainCategory, 0, _GetThreadID(),
		"Enter RES_GetCBPPreviewInfo (handle=%d)", handle);
		...
		_IsValidPartnerReservedKey(pReservedkey) != IBSU_STATUS_OK)
	{
		nRc = IBSU_ERR_NOT_SUPPORTED;
		InsertTraceLog(sTraceMainCategory, nRc, _GetThreadID(),
			"Exit RES_GetCBPPreviewInfo (handle=%d)", handle);
	...
	if( nRc == IBSU_STATUS_OK )
	{
		nRc = pIBUsbManager->m_pMainCapture->Reserved_GetCBPPreviewInfo(segmentPositionArray, segmentPositionArray_for_geo, segmentPositionArrayCount, IsFingerDetected);
	}
...
int	CIBScanUltimateApp::RES_CleanUpCBP
    (const int              handle)
{
//	CThreadSync Sync;
	...
	InsertTraceLog(sTraceMainCategory, 0, _GetThreadID(),
		"Enter RES_CleanUpCBP (handle=%d)", handle);
	...
	if( nRc == IBSU_STATUS_OK )
	{
		nRc = pIBUsbManager->m_pMainCapture->Reserved_CleanUpCBP();
	}

	InsertTraceLog(sTraceMainCategory, nRc, _GetThreadID(),
		"Exit RES_CleanUpCBP (handle=%d)", handle);

	return nRc;
}

int WINAPI RESERVED_GetCBPPreviewInfo
    (const int              handle,
	const char            *pReservedkey,
	IBSU_SegmentPosition  *segmentPositionArray,
	IBSU_SegmentPosition  *segmentPositionArray_for_geo,
	...
	BOOL				  *IsFingerDetected)
{
	...
	return theApp.RES_GetCBPPreviewInfo(handle, pReservedkey, segmentPositionArray, segmentPositionArray_for_geo, segmentPositionArrayCount, IsFingerDetected);
}

int WINAPI RESERVED_CleanUpCBP
    (const int              handle)
{
    AFX_MANAGE_STATE(AfxGetStaticModuleState());

	return theApp.RES_CleanUpCBP(handle);
}
