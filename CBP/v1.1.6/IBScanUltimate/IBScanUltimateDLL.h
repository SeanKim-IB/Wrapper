...
#ifdef _WINDOWS
	int					RES_GetSpoofScore(const char *pReservedKey, RESERVED_CaptureDeviceID  deviceID, const BYTE *pImage, const int Width, const int Height, int *pScore);
#endif
	int					RES_GetCBPPreviewInfo(const int handle, const char *pReservedkey, IBSU_SegmentPosition *segmentPositionArray, IBSU_SegmentPosition *segmentPositionArray_for_geo, int *segmentPositionArrayCount, BOOL *IsFingerDetected);
	int					RES_CleanUpCBP(const int handle);
