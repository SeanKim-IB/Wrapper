...
CLibCBPApp::CLibCBPApp()
{
	...
	// 20180828 added for debugging
	/*m_DebugLevel = 2;
	SYSTEMTIME systime;
	GetLocalTime(&systime);
	sprintf(m_DebugFileName, "./IB_CBP_Wrapper_%4ld%02ld%02ld_%02ld%02ld%02ld.log", 
		systime.wYear, systime.wMonth, systime.wDay, systime.wHour, systime.wMinute, systime.wSecond);
	TCHAR pf[MAX_PATH];
	if(SHGetSpecialFolderPath(0, pf, CSIDL_MYPICTURES, FALSE) == TRUE)
	{
		if(pf[strlen(pf)-1] == '\\')
			sprintf(m_DebugFileName, "%sIB_CBP_Wrapper_%4ld%02ld%02ld_%02ld%02ld%02ld.log", 
			pf, systime.wYear, systime.wMonth, systime.wDay, systime.wHour, systime.wMinute, systime.wSecond);
		else
			sprintf(m_DebugFileName, "%s\\IB_CBP_Wrapper_%4ld%02ld%02ld_%02ld%02ld%02ld.log",
			pf, systime.wYear, systime.wMonth, systime.wDay, systime.wHour, systime.wMinute, systime.wSecond);
	}*/
	...
	{
		if(pf[strlen(pf)-1] == '\\')
			sprintf(m_DebugFileName, "%sIB_CBP_Wrapper.log", pf);
		else
			sprintf(m_DebugFileName, "%s\\IB_CBP_Wrapper.log", pf);
	}
	
	// 20180903 changed debuglevel and debug filename to read from INI.
	if( GetCurrentDirectory(512, m_CurrentFolder) != 0)
	{
		CString inifile;
		if(m_CurrentFolder[strlen(m_CurrentFolder)-1] == '\\')
			inifile.Format("%s%s", m_CurrentFolder, INI_FILE_NAME);
		else
			inifile.Format("%s\\%s", m_CurrentFolder, INI_FILE_NAME);

		m_INIMgr.SetFilename(inifile);

		if(m_INIMgr.ISExistINI() == FALSE)
		{
			m_INIMgr.WriteProperty("DebugInfo", "Level", m_DebugLevel);
			m_INIMgr.WriteProperty("DebugInfo", "Filename", m_DebugFileName);
			m_INIMgr.WriteProperty("DebugInfo", "FileSizeLimit", m_DebugFileSizeLimit);
		}
	...
	for(int i=0; i<MAX_LOG_COUNT_INDEX+1; i++)
	{
		m_VFoundLogFiles[i] = "";
	}
...
void CLibCBPApp::OnEvent_PreviewImage(const int deviceHandle, void* pContext, const IBSU_ImageData image)
{
	...
	if( pCBPUsbDev->m_bReceivedResultImage == FALSE ) // during startimaging
	{
		...
		if(RESERVED_GetCBPPreviewInfo(deviceHandle, "ibkorea1120!", segmentPositionArray, segmentPositionArray_for_geo, &segmentPositionArrayCount, &IsFingerDetected) != IBSU_STATUS_OK)
		{
			...
			if(found_idx != -1)
			{
				...
				if(IsValidGeo == FALSE && (best_score > 25 || best_score == 0))
				{
					if(i == 3)
					{
						...
						if(pApp->m_IsValidGeoCnt > 4)
						{
							pApp->m_IsValidGeoCnt = 5;
...
void CLibCBPApp::_LogMessage(char *msg, char *flag, char *result, int requiredDebugLevel)
{
	...
	if(m_DebugLevel >= requiredDebugLevel)
	{
		...
		if(size > m_DebugFileSizeLimit)
		{
			...
			if (m_VFoundLogFilesCount > MAX_LOG_COUNT_INDEX)
			{
...
void CLibCBPApp::callback_error(int err_code, char *error_location)
{
	if(m_Callbacks.cbp_fp_onError)
	{
		...
		case CBP_FP_ERROR_ALREADY_LOCKED:
			sprintf(error_msg, "the connected scanner is already locked"); break;
		case CBP_FP_ERROR_NOT_LOCKED:
			sprintf(error_msg, "the connected scanner is not locked"); break;
		...
		case CBP_FP_ERROR_PROPERTY_NOT_SETABLE:
			sprintf(error_msg, "the property is not setable"); break;
		...
		case CBP_FP_ERROR_PROPERTY:
			sprintf(error_msg, "an error occurred during a cbp_fp_getProperty call"); break;
		case CBP_FP_ERROR_CALIBRATE:
			sprintf(error_msg, "an error occurred during the calibrate"); break;
		...
		case CBP_FP_ERROR_CONFIGURE:
			sprintf(error_msg, "an error occurred during configuration"); break;
		case CBP_FP_ERROR_GET_DIRTINESS:
			sprintf(error_msg, "an error occurred when checking for dirtiness"); break;
		case CBP_FP_ERROR_POWER_SAVE:
			sprintf(error_msg, "an error occurred during power save"); break;
		//case CBP_FP_ERROR_ALREADY_INITIALIZED:
		//	sprintf(error_msg, "the scanner is initialized already"); break;
		case CBP_FP_ERROR_ALREADY_CAPTURING:
			sprintf(error_msg, "the scanner is capturing already"); break;
		case CBP_FP_ERROR_NOT_CAPTURING:
			sprintf(error_msg, "the scanner is not capturing"); break;
		case CBP_FP_ERROR_NOW_CAPTURING:
			sprintf(error_msg, "the scanner is capturing now"); break;
...
void CLibCBPApp::DLL_setProperty(int handle, struct cbp_fp_property *property)
{
	...
	{
		callback_error(CBP_FP_ERROR_PROPERTY_NOT_SETABLE, "cbp_fp_setProperty");
	}
	else if(!strcmp(property->name, CBP_FP_PROPERTY_DEBUG_LEVEL))
	{
		...
		if(iValue >= 0 && iValue <= 2)
		{
			...
			callback_error(CBP_FP_ERROR_PROPERTY, "cbp_fp_setProperty");
	}
...
int CLibCBPApp::_AddToArr_LockInfo(int handle, long processID)
{
	...
	if(_CheckDuplicate_LockInfo(handle, processID) == TRUE)
		return CBP_FP_ERROR_ALREADY_LOCKED;
...
int CLibCBPApp::_DeleteFromArr_LockInfo(int handle, long processID)
{
	...
	return CBP_FP_ERROR_NOT_LOCKED;
}
...
void CLibCBPApp::_LogManage()
{
	...
	for (i=0; i < vector_size-MAX_LOG_COUNT_INDEX; i++)
	{
	...
	{
		if(i >= vector_size-MAX_LOG_COUNT_INDEX) 
			break;
