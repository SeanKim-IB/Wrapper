...
UINT CLibCBPApp::_THREAD_cbp_fp_calibrate(LPVOID pParam)
{
	...
	if( nRc != CBP_FP_OK )
	{
		pApp->callback_warning(CBP_FP_WARNING_NOT_LOCKED, "cbp_fp_calibrate");
		//goto done;
	}
...
UINT CLibCBPApp::_THREAD_cbp_fp_close(LPVOID pParam)
{
	...
	{
		// CBP requirement #1
		pApp->_DeleteFromArr_LockInfo(thread_param->handle, (long)GetCurrentProcessId());
...
UINT CLibCBPApp::_THREAD_cbp_fp_initialize(LPVOID pParam)
{
	...
	{
		//pApp->callback_error(CBP_FP_ERROR_ALREADY_INITIALIZED, "cbp_fp_initialize");
		pApp->m_Callbacks.cbp_fp_onWarning(CBP_FP_WARNING_ALREADY_INITIALIZED, "cbp_fp_initialize");
		isNew = FALSE;
		//goto done;
		...
		if(pCBPUsbDev->m_nDevHandle != -1)
		{
			...
			RESERVED_CBP_CleanUp(pCBPUsbDev->m_nDevHandle);
...
UINT CLibCBPApp::_THREAD_cbp_fp_startImaging(LPVOID pParam)
{
	...
	if(nRc != IBSU_STATUS_OK)
	{
		pApp->callback_warning(CBP_FP_WARNING_SET_PROPERTY, "cbp_fp_startImaging");
		//goto done;
	}
...
UINT CLibCBPApp::_THREAD_cbp_fp_stopImaging(LPVOID pParam)
{
	...
	if( nRc == IBSU_STATUS_OK && !IsActive )
	{
		pApp->callback_error(CBP_FP_ERROR_IMAGING, "cbp_fp_stopImaging");
		goto done;
	}
	...
	if( nRc == IBSU_STATUS_OK )
	{
		...
		BOOL m_bIsFingerOn = FALSE;
		for(int i=0; i<30; i++)
		{
			// CBP requirement #2
			RESERVED_CBP_IsFingerOn(thread_param->handle, &m_bIsFingerOn);

			if(pCBPUsbDev->m_bReceivedResultImage == TRUE || m_bIsFingerOn == FALSE)
			{
				break;
			}
...
UINT CLibCBPApp::_THREAD_cbp_fp_unlock(LPVOID pParam)
{
	...
	// clean up
	IBSU_CancelCaptureImage(thread_param->handle);
	RESERVED_CBP_CleanUp(thread_param->handle);
