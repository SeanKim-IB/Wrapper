...
'*                        Changed datatype of IBSM_ImageData structure at VB.Net
...
Partial Public Class DLL
    ...
    Public Const IBSU_MAX_STR_LEN As Integer = 128

    '/* Minimum value of contrast. */
    Public Const IBSU_MIN_CONTRAST_VALUE As Integer = 0

    '/* Maximum value of contrast. */
    Public Const IBSU_MAX_CONTRAST_VALUE As Integer = 34

    '/* Required buffer length for segment parameters. */
    Public Const IBSU_MAX_SEGMENT_COUNT As Integer = 4
    '/* Required buffer length for the 8bit bitmap header. */
    Public Const IBSU_BMP_GRAY_HEADER_LEN As Integer = 1078
    '/* Required buffer length for the 24bit bitmap header. */
    Public Const IBSU_BMP_RGB24_HEADER_LEN As Integer = 54
    '/* Required buffer length for the 32bit bitmap header. */
    Public Const IBSU_BMP_RGB32_HEADER_LEN As Integer = 54

    '/* Capture options.  For more information, see IBSU_BeginCaptureImage(). */
    Public Const IBSU_OPTION_AUTO_CONTRAST As Integer = 1
    Public Const IBSU_OPTION_AUTO_CAPTURE As Integer = 2
    Public Const IBSU_OPTION_IGNORE_FINGER_COUNT As Integer = 4

    '/* LED bit defines. */
    '/* All LEDs off. */
    Public Const IBSU_LED_NONE As UInteger = &H0
    '/* All LEDs on. */
    Public Const IBSU_LED_ALL As UInteger = &HFFFFFFFFUI
    '/* Reserved for vendor (user cannot control it). */
    Public Const IBSU_LED_INIT_BLUE As UInteger = &H1
    '/* OK key - green blink phase 1. */
    Public Const IBSU_LED_SCAN_GREEN As UInteger = &H2
    '/* Red LED for Curve (TBN240). */
    Public Const IBSU_LED_SCAN_CURVE_RED As UInteger = &H10
    '/* Green LED for Curve (TBN240). */
    Public Const IBSU_LED_SCAN_CURVE_GREEN As UInteger = &H20
    '/* Blue LED for Curve (TBN240). */
    Public Const IBSU_LED_SCAN_CURVE_BLUE As UInteger = &H40

    '/* Specific LED bit defines with LED type = ENUM_IBSU_LED_TYPE_FSCAN (e.g four finger scanner, Kojak). */
    ' All Green LEDs blink. 
    Public Const IBSU_LED_F_BLINK_GREEN As UInteger = &H10000000
    ' All Red LEDs blink. 
    Public Const IBSU_LED_F_BLINK_RED As UInteger = &H20000000
    ' Green LED for left little finger. 
    Public Const IBSU_LED_F_LEFT_LITTLE_GREEN As UInteger = &H1000000
    ' Red LED for left little finger. 
    Public Const IBSU_LED_F_LEFT_LITTLE_RED As UInteger = &H2000000
    ' Green LED for left ring finger. 
    Public Const IBSU_LED_F_LEFT_RING_GREEN As UInteger = &H4000000
    ' Red LED for left ring finger. 
    Public Const IBSU_LED_F_LEFT_RING_RED As UInteger = &H8000000
    ' Green LED for left middle finger. 
    Public Const IBSU_LED_F_LEFT_MIDDLE_GREEN As UInteger = &H100000
    ' Red LED for left middle finger. 
    Public Const IBSU_LED_F_LEFT_MIDDLE_RED As UInteger = &H200000
    ' Green LED for left index finger. 
    Public Const IBSU_LED_F_LEFT_INDEX_GREEN As UInteger = &H400000
    ' Red LED for left index finger. 
    Public Const IBSU_LED_F_LEFT_INDEX_RED As UInteger = &H800000
    ' Green LED for left thumb finger. 
    Public Const IBSU_LED_F_LEFT_THUMB_GREEN As UInteger = &H10000
    ' Red LED for left thumb finger. 
    Public Const IBSU_LED_F_LEFT_THUMB_RED As UInteger = &H20000
    ' Green LED for right thumb finger. 
    Public Const IBSU_LED_F_RIGHT_THUMB_GREEN As UInteger = &H40000
    ' Red LED for right thumb finger. 
    Public Const IBSU_LED_F_RIGHT_THUMB_RED As UInteger = &H80000
    ' Green LED for right index finger. 
    Public Const IBSU_LED_F_RIGHT_INDEX_GREEN As UInteger = &H1000
    ' Red LED for right index finger. 
    Public Const IBSU_LED_F_RIGHT_INDEX_RED As UInteger = &H2000
    ' Green LED for right middle finger. 
    Public Const IBSU_LED_F_RIGHT_MIDDLE_GREEN As UInteger = &H4000
    ' Red LED for right middle finger. 
    '/* 0x00008000 cannot be used at VB6 (Negative value) */
    'Public Const IBSU_LED_F_RIGHT_MIDDLE_RED As UInteger = &H8000
    Public Const IBSU_LED_F_RIGHT_MIDDLE_RED As UInteger = &H40000000
    ' Green LED for right ring finger. 
    Public Const IBSU_LED_F_RIGHT_RING_GREEN As UInteger = &H100
    ' Red LED for right ring finger. 
    Public Const IBSU_LED_F_RIGHT_RING_RED As UInteger = &H200
    ' Green LED for right little finger. 
    Public Const IBSU_LED_F_RIGHT_LITTLE_GREEN As UInteger = &H400
    ' Red LED for right little finger. 
    Public Const IBSU_LED_F_RIGHT_LITTLE_RED As UInteger = &H800
    ' Green LED for right ring finger. 
    Public Const IBSU_LED_F_PROGRESS_ROLL As UInteger = &H10
    ' Red LED for right ring finger. 
    Public Const IBSU_LED_F_PROGRESS_LEFT_HAND As UInteger = &H20
    ' Green LED for right little finger. 
    Public Const IBSU_LED_F_PROGRESS_TWO_THUMB As UInteger = &H40
    ' Red LED for right little finger. 
    Public Const IBSU_LED_F_PROGRESS_RIGHT_HAND As UInteger = &H80
    ...
    Public Const IBSU_FINGER_NONE As UInteger = &H0
    Public Const IBSU_FINGER_LEFT_LITTLE As UInteger = &H1
    Public Const IBSU_FINGER_LEFT_RING As UInteger = &H2
    Public Const IBSU_FINGER_LEFT_MIDDLE As UInteger = &H4
    Public Const IBSU_FINGER_LEFT_INDEX As UInteger = &H8
    Public Const IBSU_FINGER_LEFT_THUMB As UInteger = &H10
    Public Const IBSU_FINGER_RIGHT_THUMB As UInteger = &H20
    Public Const IBSU_FINGER_RIGHT_INDEX As UInteger = &H40
    Public Const IBSU_FINGER_RIGHT_MIDDLE As UInteger = &H80
    Public Const IBSU_FINGER_RIGHT_RING As UInteger = &H100
    Public Const IBSU_FINGER_RIGHT_LITTLE As UInteger = &H200
    Public Const IBSU_FINGER_LEFT_HAND As UInteger = (IBSU_FINGER_LEFT_INDEX Or IBSU_FINGER_LEFT_MIDDLE Or IBSU_FINGER_LEFT_RING Or IBSU_FINGER_LEFT_LITTLE)
    Public Const IBSU_FINGER_RIGHT_HAND As UInteger = (IBSU_FINGER_RIGHT_INDEX Or IBSU_FINGER_RIGHT_MIDDLE Or IBSU_FINGER_RIGHT_RING Or IBSU_FINGER_RIGHT_LITTLE)
    Public Const IBSU_FINGER_BOTH_THUMBS As UInteger = (IBSU_FINGER_RIGHT_THUMB Or IBSU_FINGER_LEFT_THUMB)
    Public Const IBSU_FINGER_ALL As UInteger = (IBSU_FINGER_LEFT_HAND Or IBSU_FINGER_RIGHT_HAND Or IBSU_FINGER_BOTH_THUMBS)
    Public Const IBSU_FINGER_LEFT_LITTLE_RING As UInteger = (IBSU_FINGER_LEFT_LITTLE Or IBSU_FINGER_LEFT_RING)
    Public Const IBSU_FINGER_LEFT_MIDDLE_INDEX As UInteger = (IBSU_FINGER_LEFT_MIDDLE Or IBSU_FINGER_LEFT_INDEX)
    Public Const IBSU_FINGER_RIGHT_INDEX_MIDDLE As UInteger = (IBSU_FINGER_RIGHT_INDEX Or IBSU_FINGER_RIGHT_MIDDLE)
    Public Const IBSU_FINGER_RIGHT_RING_LITTLE As UInteger = (IBSU_FINGER_RIGHT_RING Or IBSU_FINGER_RIGHT_LITTLE)

    <StructLayout(LayoutKind.Explicit)> _
    Public Structure IBSU_RECT
        <FieldOffset(0)> _
    ...
    '****************************************************************************************************
    '*/
    Public Structure IBSU_SdkVersion
        <MarshalAs(UnmanagedType.ByValTStr, SizeConst:=IBSU_MAX_STR_LEN)> _
        Public Product As String        '///< Product version string          
        <MarshalAs(UnmanagedType.ByValTStr, SizeConst:=IBSU_MAX_STR_LEN)> _
        Public File As String        '///< File version string
    End Structure
    ...
    '****************************************************************************************************
    '*/
    Public Structure IBSU_DeviceDesc
        <MarshalAs(UnmanagedType.ByValTStr, SizeConst:=IBSU_MAX_STR_LEN)> _
        Public serialNumber As String        '///< Device serial number
        <MarshalAs(UnmanagedType.ByValTStr, SizeConst:=IBSU_MAX_STR_LEN)> _
        Public productName As String        '///< Device product name
        <MarshalAs(UnmanagedType.ByValTStr, SizeConst:=IBSU_MAX_STR_LEN)> _
        Public interfaceType As String        '///< Device interface type (USB, Firewire)
        <MarshalAs(UnmanagedType.ByValTStr, SizeConst:=IBSU_MAX_STR_LEN)> _
        Public fwVersion As String        '///< Device firmware version
        <MarshalAs(UnmanagedType.ByValTStr, SizeConst:=IBSU_MAX_STR_LEN)> _
        Public devRevision As String        '///< Device revision
        Public handle As Integer        '///< Return device handle
        Public IsHandleOpened As Boolean        '///< Check if device handle is opened
    End Structure
    ...
    '*/
    <StructLayout(LayoutKind.Explicit)> _
    Public Structure IBSU_SegmentPosition
        <FieldOffset(0)> _
        Public x1 As Short
...
'/*
    ...
    '****************************************************************************************************
    '*/
    Public Structure IBSM_ImageData
        Public ImageFormat As IBSM_ImageFormat
        Public ImpressionType As IBSM_ImpressionType
        Public FingerPosition As IBSM_FingerPosition
        Public CaptureDeviceTechID As IBSM_CaptureDeviceTechID
        Public CaptureDeviceVendorID As UShort
        Public CaptureDeviceTypeID As UShort
        Public ScanSamplingX As UShort
        Public ScanSamplingY As UShort
        Public ImageSamplingX As UShort
        Public ImageSamplingY As UShort
        Public ImageSizeX As UShort
        Public ImageSizeY As UShort
        Public ScaleUnit As Byte
        Public BitDepth As Byte
        Public ImageDataLength As UInteger
