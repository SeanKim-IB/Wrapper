...
HBITMAP CButtonST::CreateGrayscaleBitmap(HBITMAP hBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTrans)
{
	...
	HDC			hMainDC = NULL, hhMemDc1 = NULL, hhMemDc2 = NULL;
	HBITMAP		hOldBmp1 = NULL, hOldBmp2 = NULL;

	hMainDC = ::GetDC(NULL);
	if (hMainDC == NULL)	return NULL;
	hhMemDc1 = ::CreateCompatibleDC(hMainDC);
	if (hhMemDc1 == NULL)
	{
		::ReleaseDC(NULL, hMainDC);
		return NULL;
	} // if
	hhMemDc2 = ::CreateCompatibleDC(hMainDC);
	if (hhMemDc2 == NULL)
	{
		::DeleteDC(hhMemDc1);
		::ReleaseDC(NULL, hMainDC);
	...
	if (hGrayBitmap)
	{
		hOldBmp1 = (HBITMAP)::SelectObject(hhMemDc1, hGrayBitmap);
		hOldBmp2 = (HBITMAP)::SelectObject(hhMemDc2, hBitmap);

		//::BitBlt(hhMemDc1, 0, 0, dwWidth, dwHeight, hhMemDc2, 0, 0, SRCCOPY);
		...
		for (dwLoopY = 0; dwLoopY < dwHeight; dwLoopY++)
		{
			...
			{
				crPixel = ::GetPixel(hhMemDc2, dwLoopX, dwLoopY);
				byNewPixel = (BYTE)((GetRValue(crPixel) * 0.299) + (GetGValue(crPixel) * 0.587) + (GetBValue(crPixel) * 0.114));

				if (crPixel != crTrans)
					::SetPixel(hhMemDc1, dwLoopX, dwLoopY, RGB(byNewPixel, byNewPixel, byNewPixel));
				else
					::SetPixel(hhMemDc1, dwLoopX, dwLoopY, crPixel);
			} // for
		} // for

		::SelectObject(hhMemDc1, hOldBmp1);
		::SelectObject(hhMemDc2, hOldBmp2);
	} // if
	...
	if (hGrayBitmap)
	{
		...
		for (dwLoopY = 0; dwLoopY < dwHeight; dwLoopY++)
		{
			for (dwLoopX = 0; dwLoopX < dwWidth; dwLoopX++)
			{
				...
				if (crPixel != crTrans)
					::SetPixel(hhMemDc1, dwLoopX, dwLoopY, DarkenColor(crPixel, 0.25));
				else
					::SetPixel(hhMemDc1, dwLoopX, dwLoopY, crPixel);
			} // for
		} // for
...
HICON CButtonST::CreateGrayscaleIcon(HICON hIcon)
{
	...
	hhMemDc1 = ::CreateCompatibleDC(hMainDC);
	hhMemDc2 = ::CreateCompatibleDC(hMainDC);
	if (hMainDC == NULL || hhMemDc1 == NULL || hhMemDc2 == NULL)	return NULL;
	...
	if (::GetObject(csII.hbmColor, sizeof(BITMAP), &bmp))
	{
		...
		{
			hOldBmp1 = (HBITMAP)::SelectObject(hhMemDc1, csII.hbmColor);
			hOldBmp2 = (HBITMAP)::SelectObject(hhMemDc2, csGrayII.hbmColor);

			//::BitBlt(hhMemDc2, 0, 0, dwWidth, dwHeight, hhMemDc1, 0, 0, SRCCOPY);
			...
			for (dwLoopY = 0; dwLoopY < dwHeight; dwLoopY++)
			{
				for (dwLoopX = 0; dwLoopX < dwWidth; dwLoopX++)
				{
					crPixel = ::GetPixel(hhMemDc1, dwLoopX, dwLoopY);
					byNewPixel = (BYTE)((GetRValue(crPixel) * 0.299) + (GetGValue(crPixel) * 0.587) + (GetBValue(crPixel) * 0.114));

					if (crPixel)	
						::SetPixel(hhMemDc2, dwLoopX, dwLoopY, RGB(byNewPixel, byNewPixel, byNewPixel));
					else
						::SetPixel(hhMemDc2, dwLoopX, dwLoopY, crPixel);
				} // for
			} // for
		...
		if (csGrayII.hbmColor)
		{
			...
			for (dwLoopY = 0; dwLoopY < dwHeight; dwLoopY++)
			{
				for (dwLoopX = 0; dwLoopX < dwWidth; dwLoopX++)
				{
					...
					if (crPixel)	
						::SetPixel(hhMemDc2, dwLoopX, dwLoopY, DarkenColor(crPixel, 0.25));
					else
						::SetPixel(hhMemDc2, dwLoopX, dwLoopY, crPixel);
				} // for
			} // for
