    ...
    void   *reserved)
{
    ...
    else
    {
        ...
        {
            MethodID_IBScan_DeviceDesc_DeviceDesc = getMethodID(pEnv, Class_IBScan_DeviceDesc, "<init>", "(L" STRING_CLASS_PATH ";L" STRING_CLASS_PATH ";L" STRING_CLASS_PATH ";L" STRING_CLASS_PATH ";L" STRING_CLASS_PATH ";ZI)V", &ok);
        }
...
{
	LOG(("%s\n", __FUNCTION__));
#ifdef __rooted_device__
	int nRc;
	
	nRc = system("su -c \"chmod -R 777 /dev/bus/usb/\"");
	if( nRc != 0 )
	{
		LOG(("Warning - Could not grant permissions to USB"));
	}
#endif
...
/*
 * private native void updateUsbPermission(IBScan.NativeError error);
 */
JNIEXPORT void JNICALL IBSCAN_BUILD_JNI_FNCT(updateUsbPermissionNative)(
    JNIEnv  *pEnv,
    jobject  tthis)
{
#ifdef __rooted_device__
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            deviceId       = devDesc.devID;

                            devDescJ = (*pEnv)->NewObject(pEnv, Class_IBScan_DeviceDesc, MethodID_IBScan_DeviceDesc_DeviceDesc,
                                            serialNumber, productName, interfaceType, fwVersion, devRevision, isHandleOpened, deviceId);
                            if (devDescJ == NULL)
                            {
                                setNativeError(pEnv, error, IBSU_ERR_MEM_ALLOC);
