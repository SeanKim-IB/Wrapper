...
BOOL CMainCapture::_GetOneFrameImage_Watson(unsigned char *Image, int ImgSize)
{
	...
	if(m_propertyInfo.bEnableEncryption)
	{
		...
		//clock_t start = clock();
		memcpy(m_pAlgo->m_EncryptImage, &m_pAlgo->m_Inter_Img[2], ImgSize-2);
		memcpy(m_pAlgo->m_EncryptImage500ppi, m_pAlgo->m_EncryptImage, m_pAlgo->IMG_SIZE);

		m_pAlgo->m_AESEnc->AES_Decrypt_opt(&m_pAlgo->m_Inter_Img[2], ImgSize, &Image[2]);
...
int CMainCapture::_InitializeForCISRegister_Watson(WORD rowStart, WORD colStart, WORD width, WORD height)
{
    ...
    // [Still - 1024 x 972]
	if (_MT9M_SetRegister(MT9M_ROW_START,	(WORD)rowStart - m_propertyInfo.sKojakCalibrationInfo.LensPosRow) != IBSU_STATUS_OK) goto done;		// 0x66 for 972, 0x36 for 1080
    if (_MT9M_SetRegister(MT9M_COLUMN_START,	(WORD)colStart + m_propertyInfo.sKojakCalibrationInfo.LensPosCol) != IBSU_STATUS_OK) goto done;		// 0xF0 for 1024, 0xB0 for 1152
	...
	_Initialize_Encryption_For_Watson();
...
int CMainCapture::_IsNeedInitializeCIS_Watson()
{
	...
	if(reg_val[0] != 0x0056 - m_propertyInfo.sKojakCalibrationInfo.LensPosRow ||
		reg_val[1] != 0x00D0 + m_propertyInfo.sKojakCalibrationInfo.LensPosCol ||
		reg_val[2] != 0x03FD ||
		reg_val[3] != 0x043F)
		return TRUE;
done:
...
int CMainCapture::_Initialize_Encryption_For_Watson()
{
