...
{
	if(m_pUsbDevInfo->nFpgaVersion >= _FPGA_VER_FOR_KOJAK_3_0_WITH_DAIWON_AND_70VPP_)
		m_SBDAlg->Init_Distortion_Merge_with_Bilinear_forDaiwon(96 + m_PPI_Correction_Horizontal, 90 + m_PPI_Correction_Vertical, 0, 5);
	else
...
int CIBAlgorithm::_Algo_Kojak_GetBrightWithRawImage_Rev1(BYTE *InRawImg, int *ForgraoundCNT, int *CenterX, int *CenterY, int *Bright)
{
    int y, x, yy, xx, i, j;
	int value;
	...
	int sum_x = 0, sum_y = 0;
	int BrightValue = 0;
	int m_Minus_Value = 20;
	int Threshold = 10;
	int row, col, max_count;
	unsigned char histogram[256];
	int SegmentCnt=0, Area;
	...
	*ForgraoundCNT = 0;
	...
	RIGHT = ZOOM_ENLAGE_W;
	TOP  = 0;
	BOTTOM = ZOOM_ENLAGE_H;

	memset(m_SBDAlg->m_SumGrayBuf, 0, ZOOM_ENLAGE_W*ZOOM_ENLAGE_H * sizeof(int));
	m_SBDAlg->m_SumGrayBuf[TOP*ZOOM_ENLAGE_W+LEFT] = EnlargeBuf[TOP*ZOOM_ENLAGE_W+LEFT];

	for(xx=LEFT+1; xx<RIGHT; xx++)
		m_SBDAlg->m_SumGrayBuf[TOP*ZOOM_ENLAGE_W+xx] =
		m_SBDAlg->m_SumGrayBuf[TOP*ZOOM_ENLAGE_W+xx-1] + EnlargeBuf[TOP*ZOOM_ENLAGE_W+xx];

	for(yy=TOP+1; yy<BOTTOM; yy++)
		m_SBDAlg->m_SumGrayBuf[yy*ZOOM_ENLAGE_W+LEFT] =
		m_SBDAlg->m_SumGrayBuf[(yy-1)*ZOOM_ENLAGE_W+LEFT] + EnlargeBuf[yy*ZOOM_ENLAGE_W+LEFT];

	for(yy=TOP+1; yy<BOTTOM; yy++)
	{
		for(xx=LEFT+1; xx<RIGHT; xx++)
		{
			m_SBDAlg->m_SumGrayBuf[yy*ZOOM_ENLAGE_W+xx] = m_SBDAlg->m_SumGrayBuf[yy*ZOOM_ENLAGE_W+(xx-1)]
												+ m_SBDAlg->m_SumGrayBuf[(yy-1)*ZOOM_ENLAGE_W+xx]
												+ EnlargeBuf[yy*ZOOM_ENLAGE_W+xx]
												- m_SBDAlg->m_SumGrayBuf[(yy-1)*ZOOM_ENLAGE_W+(xx-1)];
		}
	}
	...
	{
        top = i - (window_size+1);
        bottom = i + window_size;
	
		if(top < 0)
			top = 0;
		if(bottom > ZOOM_ENLAGE_H-1)
			bottom = ZOOM_ENLAGE_H-1;
		...
		{
			// to make fast
			if (EnlargeBuf[i * ZOOM_ENLAGE_W + j] == 0)
				continue;

			left = j - (window_size+1);
            right = j + window_size;

			if(left < 0)
				left = 0;
			if(right > ZOOM_ENLAGE_W-1)
				right = ZOOM_ENLAGE_W-1;

			width = right-left;

            value = (m_SBDAlg->m_SumGrayBuf[bottom * ZOOM_ENLAGE_W + right] - m_SBDAlg->m_SumGrayBuf[top * ZOOM_ENLAGE_W + right] - 
					m_SBDAlg->m_SumGrayBuf[bottom * ZOOM_ENLAGE_W + left] + m_SBDAlg->m_SumGrayBuf[top * ZOOM_ENLAGE_W + left]) 
					/ (width*height);

			mean_d_local = (m_SBDAlg->m_SumGrayBuf[bottom * ZOOM_ENLAGE_W + right] - m_SBDAlg->m_SumGrayBuf[top * ZOOM_ENLAGE_W + right] - 
							m_SBDAlg->m_SumGrayBuf[bottom * ZOOM_ENLAGE_W + left] + m_SBDAlg->m_SumGrayBuf[top * ZOOM_ENLAGE_W + left]) / (float)(width*height);
            std_d_local = (m_SBDAlg->m_SumGrayBuf2[bottom * ZOOM_ENLAGE_W + right] - m_SBDAlg->m_SumGrayBuf2[top * ZOOM_ENLAGE_W + right] - 
						  m_SBDAlg->m_SumGrayBuf2[bottom * ZOOM_ENLAGE_W + left] + m_SBDAlg->m_SumGrayBuf2[top * ZOOM_ENLAGE_W + left]) / (float)(width*height)
						  - mean_d_local * mean_d_local;
			
			if(mean_d_local < 40 && std_d_local < 2500)	// sqrt(25) = 5
			{
				value = EnlargeBuf[(y)*ZOOM_ENLAGE_W + (x)] - (int)(mean_d_local + sqrt(std_d_local)*2);
				if(value < 0)
					value = 0;
				EnlargeBuf[(y)*ZOOM_ENLAGE_W + (x)] = value;
			}
	...
	//m_SBDAlg->SaveBitmapImage("d:\\enlargebuf_1_1.bmp", EnlargeBuf, ZOOM_ENLAGE_W, ZOOM_ENLAGE_H);
	...
	for (i = ENLARGESIZE_ZOOM_H; i < ZOOM_ENLAGE_H - ENLARGESIZE_ZOOM_H; i++)
	{
		for (j = ENLARGESIZE_ZOOM_W; j < ZOOM_ENLAGE_W - ENLARGESIZE_ZOOM_W; j++)
		{
			...
			width = right-left;
 			...
 			Threshold = (m_SBDAlg->m_SumGrayBuf[bottom * ZOOM_ENLAGE_W + right] - m_SBDAlg->m_SumGrayBuf[top * ZOOM_ENLAGE_W + right] - 
									m_SBDAlg->m_SumGrayBuf[bottom * ZOOM_ENLAGE_W + left] + m_SBDAlg->m_SumGrayBuf[top * ZOOM_ENLAGE_W + left]) / (width*height);
			...
			right = j-1;
 			...
 			tempsum = (m_SBDAlg->m_SumGrayBuf[bottom * ZOOM_ENLAGE_W + right] - m_SBDAlg->m_SumGrayBuf[top * ZOOM_ENLAGE_W + right] - 
									m_SBDAlg->m_SumGrayBuf[bottom * ZOOM_ENLAGE_W + left] + m_SBDAlg->m_SumGrayBuf[top * ZOOM_ENLAGE_W + left]) / (9);
	...
	//// Labeling 후 같은 Labeling내에서만 Delation 연산
	//memcpy(m_labeled_segment_enlarge_buffer, m_segment_enlarge_buffer_for_fingercnt, ZOOM_ENLAGE_W*ZOOM_ENLAGE_H);
	...
	//	for (j=ENLARGESIZE_ZOOM_W; j<ZOOM_ENLAGE_W-ENLARGESIZE_ZOOM_W; j++)
	//	{
	//		if(m_labeled_segment_enlarge_buffer[i*ZOOM_ENLAGE_W+j] == 255)
	//		{
	//			SegmentCnt++;
	...
	//				if(m_labeled_segment_enlarge_buffer[(i-row)*ZOOM_ENLAGE_W+j+col] > 0)
	//				{
	//					histogram[m_labeled_segment_enlarge_buffer[(i-row)*ZOOM_ENLAGE_W+j+col]]++;

	//					if(max_count < histogram[m_labeled_segment_enlarge_buffer[(i-row)*ZOOM_ENLAGE_W+j+col]])
	//						max_count = histogram[m_labeled_segment_enlarge_buffer[(i-row)*ZOOM_ENLAGE_W+j+col]];
	//				}
	//			}
	//		}

	//		if (max_count >= 5*5/2-1)//4 * 255)
	//		{
	//			m_segment_enlarge_buffer_for_fingercnt[i * ZOOM_ENLAGE_W + j] = 255;
	//		}
	//	}
	...
	_Algo_Erosion(m_segment_enlarge_buffer_for_fingercnt, ZOOM_ENLAGE_W, ZOOM_ENLAGE_H);
	_Algo_Dilation(m_segment_enlarge_buffer_for_fingercnt, ZOOM_ENLAGE_W, ZOOM_ENLAGE_H);
	_Algo_Erosion(m_segment_enlarge_buffer_for_fingercnt, ZOOM_ENLAGE_W, ZOOM_ENLAGE_H);
...
BOOL CIBAlgorithm::_Algo_Kojak_AutoCapture_CurrentSensor_and_ATOF(CaptureSetting *pSetting, int frameTime, int ImgSize, int *CaptureGood,
																int *OperationMode)
{
//	int	ForgraoundCNT=0, fingerCount=0, AreaTres, RecomGain;
	int			AreaTres;
...
//	int			forSuperDry_Mode=pSetting->forSuperDry_Mode;
	...
	*CaptureGood = FALSE;
	...
	if( (*OperationMode == 2 && m_isEnabledATOF == FALSE) ||
		(*OperationMode == 1 && !m_cImgAnalysis.is_final && m_cImgAnalysis.isDetected == 0 && m_cImgAnalysis.isDetectedTOF == FALSE)
		)
	{
		pSetting->CurrentGain = pSetting->DefaultGain;
		pSetting->CurrentDAC = pSetting->DefaultDAC;
		...
		m_cImgAnalysis.JudgeMeanMode = 0;
		m_cImgAnalysis.JudgeMeanModeCount = 0;
		m_cImgAnalysis.false_contact_count = 0;
	...
	// existing tingle-fix logic
	if(*OperationMode == 1)// && m_TrueCount > 3)
	{
		m_AntiTingleMethod = 4;

		if(m_AntiTingleMethod == 1)
		{
			int DACThres = 10;
			if(m_cImgAnalysis.mean < DACThres && m_mainTOFValue > 1800 || m_mainTOFValue > 3000)//2500)
			{
		...
		else if(m_AntiTingleMethod == 2)
		{
			// 수치로 환산
			m_CurrentNormalized = _ConvertTOFtoVal(pSetting->CurrentDAC, m_mainTOFValue);
			if(pSetting->CurrentDAC >= 0xEF)
				m_CurrentNormalizedThreshold = 2600;
			else
			{
				m_CurrentNormalizedThreshold = 2600 + 10 * (0xEF-pSetting->CurrentDAC);
				if(m_CurrentNormalizedThreshold > 3000)
					m_CurrentNormalizedThreshold = 3000;
			}

			if( (m_cImgAnalysis.mean < m_DACThres && (m_mainTOFValue > 1800 || m_CurrentNormalized > 1900)) ||  
				(pSetting->CurrentDAC < 0xEF && (/*m_mainTOFValue > 3000 || */m_CurrentNormalized > m_CurrentNormalizedThreshold)) )
			{
				m_cImgAnalysis.false_contact_count++;
				if(m_cImgAnalysis.false_contact_count > 3)
				{
					m_cImgAnalysis.false_contact_count = 3;
			...
			if(m_cImgAnalysis.finger_count > m_pPropertyInfo->nNumberOfObjects+1)
			{
				pSetting->CurrentDAC += 1;
				if (pSetting->CurrentDAC > pSetting->DefaultMinDAC)
				{
			...
			else if(m_cImgAnalysis.finger_count > m_pPropertyInfo->nNumberOfObjects)
			{
				return TRUE;
			}
		}
		else if(m_AntiTingleMethod == 3)
		{
			// 수치로 환산
			m_CurrentNormalized = _ConvertTOFtoVal(pSetting->CurrentDAC, m_mainTOFValue);
			if(pSetting->CurrentDAC >= 0xF3)
				m_CurrentNormalizedThreshold = 3000;
			else
			{
				// 면적과 전압을 반영하여 Threshold를 증가시킴
				m_CurrentNormalizedThreshold = 3000 + 10 * (0xF3-pSetting->CurrentDAC) + (10 * m_cImgAnalysis.foreground_count / 500);
				if(m_CurrentNormalizedThreshold > 4500)
					m_CurrentNormalizedThreshold = 4500;
			}

			// 1. 밝기가 어두운데 전류값이 높을 경우는 기본적으로 Trick
			if( m_cImgAnalysis.mean < m_DACThres && (m_mainTOFValue > 1800 || m_CurrentNormalized > 2000) )
			{
				m_cImgAnalysis.false_contact_count++;
				if(m_cImgAnalysis.false_contact_count > 2)
				{
					m_cImgAnalysis.false_contact_count = 2;
			...
			// 2. 밝기와 상관없이 전압과 면적에 비해서 전류값이 높을 경우 Trick
			// 3. 굉장히 Wet하지 않는 이상 3000을 넘지는 않아야 함 (Trick이 아니라면).
			else if( (pSetting->CurrentDAC < 0xF3 && (m_CurrentNormalized > m_CurrentNormalizedThreshold)) || 
				m_mainTOFValue > 3000 )
			{
				m_cImgAnalysis.false_contact_count++;
				if(m_cImgAnalysis.false_contact_count > 2)
			...
			// 4. 요구하는 손가락 개수보다 2개이상 많은 입력이 들어왔을 경우 전압을 낮추기 시작함
			if(m_cImgAnalysis.finger_count > m_pPropertyInfo->nNumberOfObjects+1)
			{
				pSetting->CurrentDAC += 1;
				if (pSetting->CurrentDAC > pSetting->DefaultMinDAC)
			...
			// 5. 요구하는 손가락 개수보다 1개이상 많은 입력이 들어왔을 경우 전압 조정을 멈춤
			else if(m_cImgAnalysis.finger_count > m_pPropertyInfo->nNumberOfObjects)
			{
				m_cImgAnalysis.pre_center_x = m_cImgAnalysis.center_x;
				m_cImgAnalysis.pre_center_y = m_cImgAnalysis.center_y;
			...
			// 6. 이동량이 많은 경우 전압 조정 멈춤
			if( pSetting->CurrentDAC < 0xE0 && 
				(abs(m_cImgAnalysis.center_x-m_cImgAnalysis.pre_center_x) > 25 || 
				abs(m_cImgAnalysis.center_y-m_cImgAnalysis.pre_center_y) > 25 ))
			{
				m_cImgAnalysis.pre_center_x = m_cImgAnalysis.center_x;
				m_cImgAnalysis.pre_center_y = m_cImgAnalysis.center_y;
		...
		else if(m_AntiTingleMethod == 4)
		{
			// 영상이 어두울 경우 무조건 AT모드로 회귀
			if(m_cImgAnalysis.mean < _KOJAK30_BRIGHTNESS_THRES_FROM_CS_TO_AT)
			{
				*OperationMode = 2;
				m_isEnabledATOF = FALSE;
				pSetting->CurrentDAC = pSetting->DefaultMinDAC;
		...
		m_cImgAnalysis.JudgeMeanMode == 0)
	{
		...
		{
			m_cImgAnalysis.false_contact_count--;
			if(m_cImgAnalysis.false_contact_count < 0)
				m_cImgAnalysis.false_contact_count = 0;
			if(m_cImgAnalysis.false_contact_count > 0)
			{
				pSetting->CurrentDAC += 3;
		...
		if (m_cImgAnalysis.mean < updated_MIN_BRIGHT)
		{
			...
			if (!m_cImgAnalysis.is_complete_voltage_control)
			{
				...
				else if (m_cImgAnalysis.mean > (updated_MIN_BRIGHT - TOLERANCE*3/2))
				{
					...
					{
						//if(m_mainTOFValue > 2000)
						//	pSetting->CurrentDAC -= 2;
						//else 
							pSetting->CurrentDAC -= 5;
					}
				}
				else
				{
...
//	int			forSuperDry_Mode=pSetting->forSuperDry_Mode;
	...
	*CaptureGood = FALSE;
	...
	//if(m_cImgAnalysis.mean < DACThres && m_mainTOFValue > 1800 || m_mainTOFValue > 3000 || m_isEnabledATOF == FALSE )
/*	if( m_isEnabledATOF == FALSE )
	{
		//pSetting->CurrentDAC += 5;
		pSetting->CurrentDAC += 3;
...
#ifdef _KOJAK_ATOF_16B_
int CIBAlgorithm::_ConvertTOFtoVal(int DAC, int Current)
{
	int value = 0;

	float MultiplyArr[5] = {2.4f, 1.6f, 1.2f, 1.0f, 1.0f};
	int DACArr[5] = {0xFF, 0xDF, 0xBF, 0x9F, 0x40};
	int CurrentThres[5] = {450, 650, 850, 1050, 1750};
	int item_count = 5;
	float current_multiply = 1.0f;
	int current_Thres = 0;
	float weight;

	for(int i=0; i<item_count-1; i++)
	{
		if(DACArr[i] >= DAC && DAC >= DACArr[i+1])
		{
			weight = (float)(DACArr[i]-DAC) / (float)(DACArr[i]-DACArr[i+1]);
			current_multiply = (float)((1.0f-weight) * MultiplyArr[i] + weight * MultiplyArr[i+1]);
			current_Thres = (int)((1.0f-weight) * CurrentThres[i] + weight * CurrentThres[i+1]);
			break;
		}
	}

	value = (int)((Current - current_Thres) * current_multiply);
