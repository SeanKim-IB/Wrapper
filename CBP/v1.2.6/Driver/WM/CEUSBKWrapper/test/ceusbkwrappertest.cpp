...
#include "ceusbkwrapper.h"
    ...
    #include "DlgPrint.h"   // Replaces printf with a DlgPrintf
#endif 
...
void printMenu()
{
	...
	if (gDeviceHandle == INVALID_HANDLE_VALUE) {
		...
		if (gDeviceListSize) {
			...
			printf("cd) read a configuration descriptor\n");
...
BOOL handleCommand(char line[])
{
	...
	if (strcmp(line, "q") == 0)
		...
		gDeviceHandle != INVALID_HANDLE_VALUE &&
		gDeviceListSize > 0)
		requestConfigurationDescriptor(line + 2);
	else if (line[0] == 'o' &&
		gDeviceHandle != INVALID_HANDLE_VALUE &&
		gDeviceListSize > 0)
