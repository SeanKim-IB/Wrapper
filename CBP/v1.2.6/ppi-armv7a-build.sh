...
echo "##########################################################"
echo "# BUILD DEVICE PARALLEL LIBRARY"
echo "##########################################################"

cd    "LibDeviceParallel/jni/"

for abi in ${ABIS[@]} ; do
    declare "temp=CROSS_COMPILE_${abi//-/_}"
    export CROSS_COMPILE=${!temp}
    export ARCHABI=$abi
make  -f Makefile""_$OS"_ppi".mk clean
make  -f Makefile""_$OS"_ppi".mk all
    cp     lib/libDeviceParallel.so.stripped ../../lib/$ARCHABI/libDeviceParallel.so.stripped
    cp     lib/libDeviceParallel.so          ../../lib/$ARCHABI/libDeviceParallel.so
done
if [ -e "/usr/lib/libDeviceParallel.so" ]
then
    echo "Found 'libDeviceParallel.so' from /usr/lib folder"
else
    sudo cp   Samples/libDeviceParallel/lib/libDeviceParallel.so.stripped /usr/lib/libDeviceParallel.so
fi
...
OUTFOLDER="IBScanUltimatePI_$OS""_$VERSION"
PKG_UCLIBC="IBScanUltimatePI_$OS""_uclibc_$VERSION"
...
rm -f       $OUTFOLDER"/install/install-IBScanUltimate"
cp -R       lib     $OUTFOLDER
mkdir -p    $OUTFOLDER"/Samples/libDeviceParallel"
mkdir -p    $OUTFOLDER/Samples/Linux/TestScan
mkdir -p    $OUTFOLDER"/Samples/Java/IBScanUltimateJNI"
mkdir -p    $OUTFOLDER"/Samples/Python"

cp          "Samples/libDeviceParallel/Makefile_$OS"_ppi".mk" $OUTFOLDER"/Samples/libDeviceParallel/Makefile"
cp          "Samples/libDeviceParallel/DeviceParallel.cpp"   $OUTFOLDER"/Samples/libDeviceParallel/DeviceParallel.cpp"

cp          "Samples/Linux/TestScan/Makefile_$OS"_ppi".mk" $OUTFOLDER/Samples/Linux/TestScan/Makefile
