 ...
 *
 * A switch to use the Android backend was added to this file.
 *
...
API_EXPORTED int libusb_open(libusb_device *dev, libusb_device_handle **handle)
{
	...
	printf("** DEBUG - Calling linux usbfs open handle\n");
	r = usbi_backend->open(_handle);
	if (r < 0) {
		printf("** DEBUG - usbfs open handle failed: r=%d\n", r);
		libusb_unref_device(dev);
		free(_handle);
		return r;
	}
	printf("** DEBUG - Open handle succeecded\n");
