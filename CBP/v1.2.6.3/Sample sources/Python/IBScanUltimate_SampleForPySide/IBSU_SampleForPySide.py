...
class CaptureInfo():
    def __init__(self, PreCaptureMessage=None, PostCaptuerMessage=None, ImageType=None, NumberOfFinger=None, fingerName=None):
...
s_brushPlaten = QColor.fromRgb(78,78,78)
...
class IBSU_SampleDialog(QMainWindow, Ui_IBSU_SampleForPySide):
    ...
    def __init__(self, parent=None):
        ...
        strDrawString = ("<html><head/><body><p><span style=' font-size:13pt; color:#ffffff;'>PySide sample with DLL ver. %s</span></p></body></html>" % bytes(version.Product).decode('ascii'));
    ...
    def _AddCaptureSeqVector(self, PreCaptureMessage, PostCaptuerMessage, imageType, NumberOfFinger, fingerName):
        info = CaptureInfo()
    ...
    def _SaveJP2Image(self, image, fingerName):
        ...
        IBSU_SaveJP2Image(create_string_buffer(strFileName.encode("utf-8")), image.Buffer,
                          image.Width, image.Height, image.Pitch, image.ResolutionX, image.ResolutionY, c_int(80))
...
# /////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    @QtCore.Slot(int)
    def OnMsg_AskRecapture(self, imageStatus):
        if (QThread.currentThread() != self.thread()):
            # // not sure how efficient it is
            QMetaObject.invokeMethod(self, "OnMsg_AskRecapture", Qt.QueuedConnection, QGenericArgument('int', imageStatus))
            return

        askMsg = "[Warning = %d] Do you want a recapture?" % imageStatus
