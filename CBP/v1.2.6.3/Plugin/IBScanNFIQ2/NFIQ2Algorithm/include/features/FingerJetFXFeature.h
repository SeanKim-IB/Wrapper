...
#include "../include/libFRFXLL/FRFXLL.h"
...
public:
	...
	struct Minutia
	{
		...
		unsigned int angle; ///< angle in degrees (full range 0..360 degrees)
	...
	struct Object
	{
		COMType comType; ///< type of requested centre of mass that shall be used for computation
	...
	struct MinutiaeBlockInfo
	{
		...
		std::vector<Minutia> vecMinutiae; ///< detected minutiae in block
	};

	struct MinutiaeInObjectInfo
	{
		COMType comType; ///< type of centre of mass that was used for computation
		unsigned int width; ///< width of rectangular or diameter of circle
		unsigned int height; ///< height of rectangular of diameter of circle
		unsigned int noOfMinutiae; ///< number of minutiae inside the defined object (e.g. rectangular, circle, ...)
	};
	...
	struct FJFXROIResults
	{
		...
		Point centreOfMassMinutiae; ///< Centre of Mass based on minutiae coordinates
		Point centreOfMassGrayValues; ///< Centre of Mass based on pixel grayvalues
		std::vector<MinutiaeBlockInfo> vecROIBlocks; ///< the detected ROI (that contain at least one minutia) blocks with position, size and number of minutiae
		std::vector<MinutiaeInObjectInfo> vecNoOfMinutiaeInRectangular; ///< vector of the number of minutiae that are inside the defined rectangular
		std::vector<MinutiaeInObjectInfo> vecNoOfMinutiaeInCircle; ///< vector of the number of minutiae that are inside the defined circle
	};

	FingerJetFXFeature(bool bOutputSpeed, std::list<NFIQ::QualityFeatureSpeed> & speedValues)
...
#ifndef WITHOUT_BIOMDI_SUPPORT
	FJFXROIResults computeROI(struct finger_minutiae_data **fmds, unsigned int minCount, 
