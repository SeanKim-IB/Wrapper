	...
	const NFIQ::FingerprintImageData fingerprintImage, unsigned char templateData[], size_t & templateSize, bool & templateCouldBeExtracted)
{
	...
	NFIQ::FingerprintImageData localFingerprintImage(fingerprintImage.m_ImageWidth, fingerprintImage.m_ImageHeight, 
		fingerprintImage.m_FingerCode, fingerprintImage.m_ImageDPI);
	// copy data now
	localFingerprintImage.resize(fingerprintImage.size());
	memcpy((void*)localFingerprintImage.data(), fingerprintImage.data(), fingerprintImage.size());
	...
	fd_min_cnt_comrect200x200.featureID = "FingerJetFX_MinCount_COMMinRect200x200";
	fd_min_cnt_comrect200x200.featureDataType = NFIQ::e_QualityFeatureDataTypeDouble;
	...
	// validity check
	if (templateSize < 28)
		throw NFIQ::NFIQException(NFIQ::e_Error_FeatureCalculationError_FJFX_ISOTemplateTooSmall, "Created ISO template is too small.");

	struct finger_minutiae_record *fmr;
	if (new_fmr(FMR_STD_ISO, &fmr) < 0) 
		throw NFIQ::NFIQException(NFIQ::e_Error_FeatureCalculationError_FJFX_CannotAllocateFMR, "Could not allocate FMR.");
	...
	if (scan_fmr(&bdb, fmr) != READ_OK)
	{
		if (fmr != NULL)
			free_fmr(fmr);
		throw NFIQ::NFIQException(NFIQ::e_Error_FeatureCalculationError_FJFX_CannotInitBDB, "Could not init BDB.");
	}
	...
	if (rcount == 0)
	{
		if (fmr != NULL)
			free_fmr(fmr);
		throw NFIQ::NFIQException(NFIQ::e_Error_FeatureCalculationError_FJFX_NoMinutiaeRecords, "No minutiae records found.");
	}
	...
	if (get_fvmrs(fmr, fvmrs) != rcount)
	{
		...
		if (fmr != NULL)
			free_fmr(fmr);
		throw NFIQ::NFIQException(NFIQ::e_Error_FeatureCalculationError_FJFX_CannotGetMinutiaeRecords, "Cannot get minutiae records.");
	}
	...
	if (get_fmds(fvmrs[0], fmds) != minCnt)
	{
		...
		if (fmr != NULL)
			free_fmr(fmr);
		throw NFIQ::NFIQException(NFIQ::e_Error_FeatureCalculationError_FJFX_CannotGetMinutiaeData, "Cannot get minutiae data.");
	}
	...
	FingerJetFXFeature::FJFXROIResults roiResults = computeROI(fmds, minCnt, 32, 
		fingerprintImage, vecRectDimensions);
	double noOfMinInRect200x200 = 0;
	for (unsigned int i = 0; i < roiResults.vecNoOfMinutiaeInRectangular.size(); i++)
	{
	...
	return featureDataList;
#else
	throw NFIQ::NFIQException(NFIQ::e_Error_FunctionNotImplemented, "libbiomdi support is not enabled. FJFX feature computation is not possible.");
#endif
}
																  ...
																  int bs, const NFIQ::FingerprintImageData & fingerprintImage, std::vector<FingerJetFXFeature::Object> vecRectDimensions)
{
	...
	// get number of minutiae that lie inside a block defined by the Centre of Mass (COM)
	...
	for (unsigned int i = 0; i < vecRectDimensions.size(); i++)
	{
		...
		if (vecRectDimensions.at(i).comType == e_COMType_MinutiaeLocation)
		{
			centre_x = x;
			centre_y = y;
		}

		unsigned int comRectHeightHalf = vecRectDimensions.at(i).height / 2; // round down
		unsigned int comRectWidthHalf = vecRectDimensions.at(i).width / 2; // round down
		// check boundaries
