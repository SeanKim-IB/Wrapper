...
{
#define HORIZONTAL_START_DEFAULT			0x0167
#define VERTICAL_START_DEFAULT				0x0032
	WORD temp;
	int		col_W;
	BYTE	col_H = 0, col_L = 0;

	int		row_W;
	BYTE	row_H = 0, row_L = 0;
...
//	temp = (m_UsbDeviceInfo.nUsbSpeed < USB_HIGH_SPEED) ? 0x0120u : 0x04FFu;	
	...
	// Window Positioning
	col_W	= HORIZONTAL_START_DEFAULT + m_propertyInfo.sKojakCalibrationInfo.LensPosCol;
	col_H	= (col_W >> 8) & 0xFF;
	col_L	= col_W & 0xFF;

	row_W	= VERTICAL_START_DEFAULT - m_propertyInfo.sKojakCalibrationInfo.LensPosRow;
	row_H	= (row_W >> 8) & 0xFF;
	row_L	= row_W & 0xFF;

    if (_Seti_SetRegister(0x41, col_H) != IBSU_STATUS_OK) goto done; //Horizontal Start H
    if (_Seti_SetRegister(0x42, col_L) != IBSU_STATUS_OK) goto done; //Horizontal Start L
    ...
    if (_Seti_SetRegister(0x45, row_H) != IBSU_STATUS_OK) goto done; //Vertical Start H
    if (_Seti_SetRegister(0x46, row_L) != IBSU_STATUS_OK) goto done; //Vertical Start L
...
void CMainCapture::_PostImageProcessing_ForPreview_Columbo(BYTE *InImg, BYTE *OutImg, BOOL bIsGoodImage)
{
	m_pAlgo->m_cImgAnalysis.TOP = 1;
	m_pAlgo->m_cImgAnalysis.LEFT = 1;
	m_pAlgo->m_cImgAnalysis.BOTTOM = m_pAlgo->CIS_IMG_H-2;
	m_pAlgo->m_cImgAnalysis.RIGHT = m_pAlgo->CIS_IMG_W-2;
