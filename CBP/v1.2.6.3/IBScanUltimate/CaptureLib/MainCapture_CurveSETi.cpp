...
//	int nHeight = m_UsbDeviceInfo.CisImgHeight;    
	int nSHor = (1600-nWidth)/2-1+colStart - m_propertyInfo.sKojakCalibrationInfo.LensPosRow;	// CIS SMT가 세로로 되어 있기때문에 반대로 적용하고 vertical Flip을 하기 때문에 부호를 반대로 적용
	int nSVer = (1200-nHeight)/2-1+rowStart - m_propertyInfo.sKojakCalibrationInfo.LensPosCol;	// 
	int nShValue;
	int nSlValue;
	int nWhValue;
	int nWlValue;
