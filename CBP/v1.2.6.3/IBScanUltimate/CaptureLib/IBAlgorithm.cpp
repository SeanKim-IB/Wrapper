...
//		arrPos_Kojak = BLOCKALLOC( DWORD, IMG_SIZE + 1);		// "+1" is checksum
	...
	m_EncryptImage = BLOCKALLOC( BYTE, CIS_IMG_SIZE );
	m_EncryptTemp = BLOCKALLOC( BYTE, CIS_IMG_SIZE );
	m_EncryptImage500ppi = BLOCKALLOC( BYTE, CIS_IMG_SIZE );
...
CIBAlgorithm::~CIBAlgorithm(void)
{
    ...
    BLOCKFREE( m_EncryptImage );
    BLOCKFREE( m_EncryptTemp );
    BLOCKFREE( m_EncryptImage500ppi );
...
void CIBAlgorithm::_Algo_Init_Distortion_Merge_with_Bilinear()
{
    ...
    int DISTORTION_SX = 0;
    int DISTORTION_SY = 0;
    int DISTORTION_RX = (CIS_IMG_W - 4 - m_PPI_Correction_Horizontal) / 2;
    int DISTORTION_RY = (CIS_IMG_H - 4 - m_PPI_Correction_Vertical) / 2;
    ...
    int shifty = (m_PPI_Correction_Vertical+4)/2;
	int shiftx = (m_PPI_Correction_Horizontal+4)/2;
...
{
	int i, j;
	unsigned char *TmpBuf = m_Inter_Img5;// unsigned char [width*height];
