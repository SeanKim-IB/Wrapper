...
DWORD WINAPI wince_clock_gettime_threaded(void* param);
...
static int wince_init(struct libusb_context *ctx)
{
	...
	if ( ++concurrent_usage == 0 ) {	// First init?
		...
		{
			DWORD dwThreadId = 0;
			timer_thread = CreateThread(NULL, 0, wince_clock_gettime_threaded, NULL, 0, &dwThreadId);
			usbi_info(ctx, "CreateThread returned 0x%x, id 0x%x", timer_thread, dwThreadId);
		}
		...
		if (WaitForSingleObject(timer_response, 60000) != WAIT_OBJECT_0) {
...
static void wince_transfer_callback(struct usbi_transfer *itransfer, uint32_t io_result, uint32_t io_size)
{
	...
	usbi_dbg("handling I/O completion for 0x%x with errcode %d, size 0x%x", itransfer, io_result, io_size);
