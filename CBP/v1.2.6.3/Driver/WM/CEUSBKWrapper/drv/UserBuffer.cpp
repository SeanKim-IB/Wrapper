...
{
	if (!mlpSrcUnmarshalled){
		WARN_MSG((TEXT("USBKWrapperDrv!UserBuffer::UserBuffer<T>() null ptr\r\n")));
		return;
	}
	if (mSize == 0)
		return;
	mlpSyncMarshalled = MapCallerPtr(lpSrcUnmarshalled, mSize);
	BUFFERMAPPING_MSG((TEXT("USBKWrapperDrv!UserBuffer::UserBuffer<T>() map caller buffer 0x%08x to 0x%08x, size=0x%x\r\n"),
			lpSrcUnmarshalled, mlpSyncMarshalled, mSize));
	...
	if (mlpSyncMarshalled != NULL && mAsync) {
		...
		if (!mlpAsyncMarshalled) {
			ERROR_MSG((TEXT("USBKWrapperDrv!UserBuffer::UserBuffer<T>() failed to alloc async buffer 0x%08x\r\n"),
				lpSrcUnmarshalled));
			mlpAsyncMarshalled = NULL;
		} else {
			/* MapCallerPtr has checked that this is a valid user space region. */
			BUFFERMAPPING_MSG((TEXT("USBKWrapperDrv!UserBuffer::UserBuffer<T>() caller buffer 0x%08x mapped to 0x%08x being copied to asyc 0x%08x, length=0x%x\r\n"),
			lpSrcUnmarshalled, mlpSyncMarshalled, mlpAsyncMarshalled, mSize));
...
UserBuffer<T>::~UserBuffer()
{
	BUFFERMAPPING_MSG((TEXT("USBKWrapperDrv!UserBuffer::~UserBuffer<T>() for user buffer 0x%08x free async buffer 0x%08x\r\n"),
	mlpSyncMarshalled, mlpAsyncMarshalled));
...
BOOL UserBuffer<T>::Flush()
{
	...
	BUFFERMAPPING_MSG((TEXT("USBKWrapperDrv!UserBuffer<T>::Flush memcopy to 0x%x from 0x%x size 0x%x\r\n"), mlpSyncMarshalled, mlpAsyncMarshalled, mSize));
...
, mCompleted(FALSE)
{
	if (!Valid() || Ptr() == NULL)
		return;
	BUFFERMAPPING_MSG((TEXT("USBKWrapperDrv!OverlappedUserBuffer::OverlappedUserBuffer - created this=0x%x\r\n"), this));
...
{
	BUFFERMAPPING_MSG((TEXT("USBKWrapperDrv!OverlappedUserBuffer::Complete this=0x%x, status=0x%x, dwBytesTransferd=0x%x\r\n"), this, dwStatus, dwBytesTransferred));
	...
	BUFFERMAPPING_MSG((TEXT("USBKWrapperDrv!OverlappedUserBuffer::Complete this=0x%x, calling Flush\r\n"), this));
	Flush();
	SetEvent(mhEvent);
...
void OverlappedUserBuffer::Abort()
{
	if (Completed() || !Valid() || Ptr() == NULL)
		return;
	BUFFERMAPPING_MSG((TEXT("USBKWrapperDrv!OverlappedUserBuffer::Abort this=0x%x\r\n"), this));
	CloseHandle(mhEvent);
	mhEvent = INVALID_HANDLE_VALUE;
	mCompleted = TRUE;
}