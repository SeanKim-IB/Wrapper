...
void Transfer::DoTransferCompleted()
{
	...
	if (!mDevicePtr->GetTransferStatusNoLock(mTransfer, &bytesTransferred, &transferError)) {
		ERROR_MSG((TEXT("USBKWrapperDrv!Transfer::TransferCompleted() used invalid transfer handle\r\n")));
	...
	TRANSFERLIFETIME_MSG((
		TEXT("USBKWrapperDrv!Transfer::TransferCompleted() completed (error %d, untranslated error %d, transferred %d, cancelled %d)\r\n"),
		translatedError, transferError, bytesTransferred, mCancelled));

#ifdef DEBUG
	if( bytesTransferred )
	{
		UsbDeviceDumpOutput( "USBKWrapperDrv!Transfer::TransferCompleted buffer", (UCHAR *)(mUserBuffer.Ptr()), bytesTransferred );
	}
#endif

	// Need to flush the IO buffer before completing the overlapped buffer
	if( translatedError == USB_NO_ERROR && bytesTransferred != 0 )  // JES 3/11/2014
	{
		mUserBuffer.Flush();		// JES 2/25/2014
	}
...
void Transfer::SetBytesTransferred(DWORD bytesTransferred)
{
	...
	FUNCTION_MSG((TEXT("USBKWrapperDrv!Transfer::SetBytesTransferred ptr=0x%x\r\n"), ptr));
...
DWORD Transfer::TranslateError(DWORD dwUsbError, DWORD dwBytesTransferred, BOOL Cancelled)
{
	...
	case USB_CANCELED_ERROR:
		// If one or more bytes have been transferred, report success
		// and represent this as a short transfer.
